...
alter table public.players
  ...
  add constraint players_gender_check check (gender is null or gender in ('male', 'female'));

drop function if exists public.get_performance_rankings(text, text, integer);
create function public.get_performance_rankings(p_category text, p_record_kind text, p_year integer default null)
...
language sql security definer set search_path = public stable as $$
  with caller as (
    select p.program_class, p.gender from public.players p where p.user_id = auth.uid() limit 1
  ), personal_bests as (
    ...
    where r.category = p_category and r.record_kind = p_record_kind
      ...
      and c.gender is not null and p.gender = c.gender
...
drop function if exists public.get_performance_leaderboard(text, text, integer);
create function public.get_performance_leaderboard(p_category text, p_record_kind text, p_year integer default null)
...
language sql security definer set search_path = public stable as $$
  with caller as (
    select program_class, gender from public.players where user_id = auth.uid() limit 1
  ), personal_bests as (
