const CACHE_NAME = "vaultex-shell-v3";
...
self.addEventListener("fetch", (event) => {
  ...
  if (["style", "script", "image", "font"].includes(event.request.destination)) {
    event.respondWith(
      fetch(event.request)
        .then((response) => {
          if (url.pathname.startsWith("/") && response.ok && url.pathname !== "/offline") {
...
self.addEventListener("push", (event) => {
  ...
  event.waitUntil(self.registration.showNotification(
    data.title ?? "SHONAI VAULTEX",
    {
      body: data.body ?? "新しいお知らせがあります。",
      icon: "/icon-192.png",
      badge: "/icon-192.png",
      data: { url: data.url ?? "/mypage" },
      tag: data.tag ?? "vaultex-notification",
      renotify: true,
    }
  ));
});
