...
const quickPrograms = [
  ...
  { key: "basic", icon: CalendarClock, label: "水曜・走り方／パフォーマンス", description: "毎週水曜 17:00〜19:00・全クラス", title: "走り方・パフォーマンスセッション", details: "前半はウォームアップと基本動作を行い、後半は走り方・基礎、ELITE、MASTERSの目的別グループに分かれて進めます。初心者から競技力向上、健康・競技継続まで、それぞれの段階に合わせて参加できます。", weekday: 3, hour: 17, minute: 0, duration: 120, scheduleType: "practice", repeat: "weekly", weeks: 12 },
  { key: "class-tuesday", icon: BookmarkPlus, label: "火曜・クラス別セッション", description: "毎週火曜 17:00〜19:00", title: "クラス別セッション（火曜）", weekday: 2, hour: 17, minute: 0, duration: 120, scheduleType: "practice", repeat: "weekly", weeks: 12, audience: "class" },
  { key: "class-thursday", icon: BookmarkPlus, label: "木曜・クラス別セッション", description: "毎週木曜 17:00〜19:00", title: "クラス別セッション（木曜）", weekday: 4, hour: 17, minute: 0, duration: 120, scheduleType: "practice", repeat: "weekly", weeks: 12, audience: "class" },
  { key: "control", icon: Gauge, label: "隔週・CONTROL TEST", description: "隔週日曜 9:00〜12:00", title: "CONTROL TEST / SCAN", weekday: 0, hour: 9, minute: 0, duration: 180, scheduleType: "measurement", repeat: "biweekly", weeks: 12 },
  ...
  { key: "personal-saturday", icon: PersonStanding, label: "土曜午後・パーソナル枠", description: "毎週土曜 13:00／14:00／15:00", title: "パーソナルセッション", weekday: 6, hour: 13, minute: 0, duration: 60, scheduleType: "other", repeat: "weekly", weeks: 12, secondHour: 14, thirdHour: 15 },
  { key: "personal-sunday", icon: PersonStanding, label: "日曜午後・パーソナル枠", description: "毎週日曜 13:00／14:00／15:00", title: "パーソナルセッション", weekday: 0, hour: 13, minute: 0, duration: 60, scheduleType: "other", repeat: "weekly", weeks: 12, secondHour: 14, thirdHour: 15 },
] as const;
...
export default function CoachScheduleManager({ initialItems, initialTemplates, initialAttendance, competitionApplicants, initialDate, initialEditingId, showScheduleList = true }: { initialItems: ScheduleItem[]; initialTemplates: ScheduleTemplate[]; initialAttendance: Array<{ schedule_id: number; status: string }>; competitionApplicants: CompetitionApplicant[]; initialDate?: string; initialEditingId?: number | null; showScheduleList?: boolean }) {
  ...
  const [thirdSlotHour, setThirdSlotHour] = useState<number | null>(null);
  ...
  const occurrenceLocals = useMemo(() => {
    ...
    if (repeat === "weekly" || repeat === "biweekly") {
      ...
      const count = Math.max(0, Math.min(104, Math.floor((until.getTime() - start.getTime()) / (intervalDays * 86400000)) + 1));
      return Array.from({ length: count }, (_, index) => { const date = new Date(start); date.setUTCDate(date.getUTCDate() + index * intervalDays); return `${date.toISOString().slice(0, 10)}T${startsAt.slice(11, 16)}`; });
    }
  ...
  const occurrenceCount = occurrenceLocals.length * (1 + Number(secondSlotHour !== null) + Number(thirdSlotHour !== null));
  const weekday = startsAt ? weekdayLabels[dateOnlyUtc(startsAt).getUTCDay()] : "—";
  function reset() { setOpen(false); setEditingId(null); setSelectedTemplate(""); setTitle(""); setDetails(""); setLocation(""); setStartsAt(""); setEndsAt(""); setAllDay(false); setSecondSlotHour(null); setThirdSlotHour(null); setIsPersonalSlot(false); setNotifyMembers(false); setTrainingPhase("normal"); setScheduleType("practice"); setAudience("all"); setRepeat("once"); setRepeatUntil(""); setRegistrationEnabled(false); setRegistrationOpensAt(""); setRegistrationDeadline(""); }
  ...
  function applyQuickProgram(program: (typeof quickPrograms)[number]) {
    reset();
    const start = nextWeekdayLocal(program.weekday, program.hour, program.minute);
    ...
    setTitle(program.title); setDetails("details" in program ? program.details : ""); setScheduleType(program.scheduleType); setStartsAt(start); setEndsAt(localValue(end.toISOString())); setRepeat(program.repeat); setRepeatUntil(until.toISOString().slice(0, 10)); setAudience("audience" in program ? program.audience : "all"); setSecondSlotHour("secondHour" in program ? program.secondHour : null); setThirdSlotHour("thirdHour" in program ? program.thirdHour : null); setIsPersonalSlot("secondHour" in program); setOpen(true);
  }
  ...
  async function submit(event: FormEvent) {
    ...
    if (editingId) {
      ...
      const registrationLocals = occurrenceLocals.flatMap((occurrenceLocal) => [occurrenceLocal, ...(secondSlotHour === null ? [] : [`${occurrenceLocal.slice(0, 10)}T${String(secondSlotHour).padStart(2, "0")}:00`]), ...(thirdSlotHour === null ? [] : [`${occurrenceLocal.slice(0, 10)}T${String(thirdSlotHour).padStart(2, "0")}:00`])]);
    ...
    const summary = repeat !== "once" && !editingId ? `${startsAt.slice(0, 10).replaceAll("-", "/")}から${repeatUntil.replaceAll("-", "/")}まで、${repeat === "monthly" ? `毎月第${Math.floor((dateOnlyUtc(startsAt).getUTCDate() - 1) / 7) + 1}${weekday}曜日` : repeat === "biweekly" ? `隔週${weekday}曜日` : `毎週${weekday}曜日`} ${startsAt.slice(11, 16)}（全${savedOccurrenceCount}回）` : `${formatJapan(start)} ${location || "場所未定"}`;
    if (notifyMembers) {
