...
const quickPrograms = [
  ...
  { key: "open", icon: CalendarPlus, label: "日曜・オープントレーニング", description: "毎週日曜 9:00〜11:00", title: "オープントレーニング", weekday: 0, hour: 9, minute: 0, duration: 120, scheduleType: "practice", repeat: "weekly", weeks: 12 },
  { key: "basic", icon: CalendarClock, label: "平日・基礎セッション", description: "毎週水曜 18:00〜19:30", title: "走り方・基礎セッション", weekday: 3, hour: 18, minute: 0, duration: 90, scheduleType: "practice", repeat: "weekly", weeks: 12 },
  { key: "control", icon: Gauge, label: "月1回・CONTROL TEST", description: "第1日曜 9:00〜11:00", title: "CONTROL TEST / SCAN", weekday: 0, hour: 9, minute: 0, duration: 120, scheduleType: "measurement", repeat: "monthly", weeks: 24 },
] as const;
...
export default function CoachScheduleManager({ initialItems, initialTemplates, initialAttendance, competitionApplicants, initialDate, initialEditingId, showScheduleList = true }: { initialItems: ScheduleItem[]; initialTemplates: ScheduleTemplate[]; initialAttendance: Array<{ schedule_id: number; status: string }>; competitionApplicants: CompetitionApplicant[]; initialDate?: string; initialEditingId?: number | null; showScheduleList?: boolean }) {
  ...
  function applyQuickProgram(program: (typeof quickPrograms)[number]) {
    ...
    setTitle(program.title); setScheduleType(program.scheduleType); setStartsAt(start); setEndsAt(localValue(end.toISOString())); setRepeat(program.repeat); setRepeatUntil(until.toISOString().slice(0, 10)); setAudience("all"); setOpen(true);
  }
