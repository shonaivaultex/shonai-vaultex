...
export default function ScheduleAttendance({ scheduleId, scheduleType, onSaved }: { scheduleId: number; scheduleType?: string; onSaved?: (status: AttendanceStatus) => void }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const [{ data, error }, { data: schedule, error: scheduleError }] = await Promise.all([supabase.from("schedule_attendance").select("status, comment").eq("schedule_id", scheduleId).eq("user_id", user.id).maybeSingle(), supabase.from("schedules").select("is_personal_slot").eq("id", scheduleId).maybeSingle()]);
      if (error || scheduleError || !schedule) { if (active) setNotice("出欠を読み込めませんでした。ページを再読み込みしてください。"); return; }
