...
import { compositeAthleteType, type ContactProfileCode } from "@/lib/contact-profile";
...
type ContactResult={code:ContactProfileCode;nameJa?:string;contactTimeMs?:number|null;contact_time_ms?:number|null;description?:string;version?:string|null};
...
export default function AthleteScanResult({evaluation,comparison,scanNumber,measuredOn,standardLabel,showReveal=false,contactProfile,jumpTrials=[]}:{evaluation:AthleteScanEvaluation;comparison:Comparison;scanNumber:number;measuredOn:string;standardLabel:string;showReveal?:boolean;contactProfile?:ContactResult|null;jumpTrials?:JumpTrial[]}) {
  ...
  const contactCode=contactProfile?.code??"NOT_MEASURED";
  const combined=compositeAthleteType(evaluation.typeCode,contactCode);
  const cmj=[...jumpTrials].filter((item)=>item.test_code==="vertical_jump"&&item.is_valid).sort((a,b)=>Number(b.jump_height_cm)-Number(a.jump_height_cm));
  const dj=[...jumpTrials].filter((item)=>item.test_code==="drop_jump"&&item.is_valid).sort((a,b)=>Number(b.rsi??0)-Number(a.rsi??0));
  return <div className="scan-result-enter space-y-7">
    <section className="overflow-hidden rounded-3xl border border-orange-500/45 bg-[radial-gradient(circle_at_top_right,rgba(249,115,22,.2),transparent_38%),#111] p-5 sm:p-8">
      ...
      <div className="mt-6 border-t border-white/10 pt-5"><p className="text-[10px] font-black tracking-[.18em] text-white/45">CURRENT ATHLETE TYPE</p><p className="mt-1 text-xs text-white/35">現在の身体特性</p>{evaluation.typeCode?<><p className="mt-3 break-words text-3xl font-black leading-tight text-orange-400 sm:text-5xl">{combined??evaluation.typeCode}</p><h2 className="mt-1 text-xl font-black sm:text-2xl">{evaluation.typeNameJa}{contactCode!=="NOT_MEASURED"?` × ${contactCode}`:""}</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-white/60">{typeCopy[evaluation.typeCode]??evaluation.typeDescription}</p></>:<><h2 className="mt-3 text-2xl font-black">PROFILE ANALYZING</h2><p className="mt-2 text-sm leading-6 text-white/50">タイプ確定に必要な測定値またはSTANDARDが不足しています。未測定は0点として扱いません。</p></>}</div>
    ...
    <section className="grid gap-4 lg:grid-cols-2"><article className="rounded-3xl border border-cyan-400/25 bg-cyan-400/[.045] p-5"><p className="text-[10px] font-black tracking-[.2em] text-cyan-300">CONTACT PROFILE / BETA</p><h2 className="mt-2 text-3xl font-black">{contactCode}</h2><p className="mt-1 font-bold">{contactProfile?.nameJa??(contactCode==="NOT_MEASURED"?"未測定":"接地特性")}</p><p className="mt-3 text-sm leading-6 text-white/55">{contactProfile?.description??(contactCode==="NOT_MEASURED"?"有効なドロップジャンプ代表試技を測定すると表示されます。":"今回の接地時間を中立的に分類したもので、優劣や競技適性を断定しません。")}</p><p className="mt-3 text-xs text-white/35">代表接地時間：{contactProfile?.contactTimeMs??contactProfile?.contact_time_ms??"—"}{contactCode!=="NOT_MEASURED"?" ms":""}</p></article><article className="rounded-3xl border border-white/10 bg-[#111] p-5"><p className="text-[10px] font-black tracking-[.2em] text-orange-400">JUMP PROFILE</p><h2 className="mt-2 text-xl font-black">CMJ / DJ / RJ</h2><div className="mt-4 grid gap-2"><Data label="CMJ ベスト" value={cmj[0]?`${cmj[0].jump_height_cm} cm`:"NOT MEASURED"}/><Data label="DJ ベストRSI" value={dj[0]?`${Number(dj[0].rsi).toFixed(2)}（${dj[0].contact_time_ms} ms）`:"NOT MEASURED"}/><Data label="RJ" value={evaluation.abilities.reactive_performance.rawValue==null?"NOT MEASURED":`${evaluation.abilities.reactive_performance.rawValue} RJ-index`}/></div><p className="mt-3 text-xs leading-5 text-white/35">CMJ・DJはVAULTEX SCOREの6能力へ加点せず、接地特性と跳躍プロフィールの確認に使用します。</p></article></section>
