...
type RecordItem = { id: number; value: number | string; date: string; awareness_category?: string | null; awareness_note?: string | null; video_path?: string | null };
...
export default function PerformanceHistoryModal({ records, unit }: { records: RecordItem[]; unit: string }) {
  ...
  return <>
    <button type="button" onClick={() => setOpen(true)} className="w-full border-t border-white/10 px-6 py-4 text-left text-sm font-bold text-orange-400 hover:bg-white/[0.025]">▶ 履歴を見る（{records.length}件）</button>
    {open && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/85 p-4 backdrop-blur-sm sm:p-8" role="dialog" aria-modal="true" onClick={() => setOpen(false)}>
      <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-orange-500/60 bg-[#111]" onClick={(event) => event.stopPropagation()}>
        <div className="sticky top-0 z-10 flex items-center justify-between border-b border-white/10 bg-[#111] px-6 py-4"><h2 className="text-lg font-black text-white">記録履歴</h2><button type="button" onClick={() => setOpen(false)} aria-label="履歴を閉じる" className="grid h-10 w-10 place-items-center rounded-full bg-white/10 text-white"><X /></button></div>
        <div className="px-6 pb-3">{records.map((record) => <div key={record.id} className="flex items-start justify-between gap-4 border-b border-white/10 py-5 last:border-0">
