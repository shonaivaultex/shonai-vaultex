...
type RecordItem = { id: number; value: number | string; date: string; wind_speed?: number | string | null; awareness_category?: string | null; awareness_categories?: string[] | null; awareness_note?: string | null; video_path?: string | null; video_url?: string | null; coach_feedback?: CoachFeedback[]; feedback_request?: FeedbackRequest | null; performance_record_details?: CompetitionDetail[] | null };
      ...
      <div className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-orange-500/60 bg-[#111]" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="px-6 pb-3">{records.map((record) => <div id={`feedback-record-${record.id}`} key={record.id} className={`border-b border-white/10 py-5 last:border-0 ${focusRecordId === record.id ? "scroll-mt-20 rounded-xl bg-orange-500/[0.06] px-3" : ""}`}>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0"><strong className="text-xl text-white">{record.value}<span className="ml-1 text-sm">{unit}</span></strong><p className="mt-1 text-xs text-white/50">{record.date}</p>{(record.awareness_categories?.length || record.awareness_category) ? <div className="mt-3"><p className="text-[9px] font-black tracking-[.14em] text-white/35">今日の感覚・意識</p><div className="mt-1.5 flex flex-wrap gap-1">{(record.awareness_categories?.length ? record.awareness_categories : record.awareness_category ? [record.awareness_category] : []).map((tag) => <span key={tag} className="inline-flex rounded-full border border-orange-500/35 bg-orange-500/10 px-2.5 py-1 text-xs font-bold text-orange-300">{tag}</span>)}</div></div> : null}{record.awareness_note && <div className="mt-3"><p className="text-[9px] font-black tracking-[.14em] text-white/35">練習の振り返り</p><p className="mt-1.5 whitespace-pre-wrap text-sm leading-6 text-white/70">{record.awareness_note}</p></div>}</div>
