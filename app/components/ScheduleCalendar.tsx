...
export default function ScheduleCalendar({ items, periods, applications, currentTime, canManage = false, coachId, initialSelectedDate, attendanceRosters = {} }: { items: ScheduleItem[]; periods: SchedulePeriod[]; applications: CompetitionApplicationItem[]; currentTime: string; canManage?: boolean; coachId?: string; initialSelectedDate?: string; attendanceRosters?: Record<number, CoachAttendanceRoster> }) {
  ...
  return <>
    ...
    <section className="mt-5 rounded-2xl border border-white/10 bg-[#111] p-5 text-white sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-black">{new Date(`${selectedDate}T00:00:00`).toLocaleDateString("ja-JP", { month: "long", day: "numeric", weekday: "long" })}の予定</h2><div className="flex flex-wrap gap-2">{canManage ? <Link href={`/mypage/schedules?newDate=${selectedDate}#schedule-management`} className="inline-flex items-center gap-1.5 rounded-lg bg-orange-500 px-3 py-2 text-xs font-black text-black"><Plus size={15}/>この日に追加</Link> : null}</div></div>{selectedItems.length ? <div className="mt-4 divide-y divide-white/10">{selectedItems.map((item) => <div key={item.id} className="pb-4"><ScheduleCard item={item} showAuthor={canManage} coachAttendanceRoster={canManage ? attendanceRosters[item.id] : undefined} />{canManage && item.author_id === coachId ? <div className="mb-3 ml-[72px] flex flex-wrap gap-2"><Link href={`/mypage/schedules?edit=${item.id}#schedule-management`} className="inline-flex items-center gap-1.5 rounded-lg border border-orange-500/35 px-3 py-2 text-xs font-bold text-orange-300"><Pencil size={14}/>この予定を編集</Link><button type="button" disabled={deletingId === item.id} onClick={() => removeSchedule(item)} className="inline-flex items-center gap-1.5 rounded-lg border border-red-500/35 px-3 py-2 text-xs font-bold text-red-300 disabled:opacity-40"><Trash2 size={14}/>{deletingId === item.id ? "削除中…" : "この予定を削除"}</button></div> : null}{item.details && <p className="mb-4 ml-[72px] whitespace-pre-wrap text-sm leading-6 text-white/55">{item.details}</p>}{item.schedule_type === "competition" && item.registration_enabled ? <div className="ml-[72px]"><CompetitionApplication scheduleId={item.id} opensAt={item.registration_opens_at} deadline={item.registration_deadline} currentTime={currentTime} initialApplication={applications.find((application) => application.schedule_id === item.id)}/></div> : null}</div>)}</div> : <p className="mt-4 text-sm text-white/35">この日の予定はありません</p>}</section>
  </>;
}
