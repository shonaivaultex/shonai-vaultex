...
const labels: Record<string, string> = { all: "すべて", practice: "練習", competition: "大会", measurement: "測定", other: "その他" };
const weekdays = ["日", "月", "火", "水", "木", "金", "土"];
function dateKey(date: Date) { return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`; }
function startOfMonth(date: Date) { return new Date(date.getFullYear(), date.getMonth(), 1); }

export default function ScheduleCalendar({ items }: { items: ScheduleItem[] }) {
  const today = new Date(); const [month, setMonth] = useState(startOfMonth(today)); const [selectedDate, setSelectedDate] = useState(dateKey(today)); const [type, setType] = useState("all"); const [audience, setAudience] = useState("both");
  ...
  return <>
    ...
    <section className="mt-5 rounded-2xl border border-white/10 bg-[#111] p-5 text-white sm:p-6"><h2 className="font-black">{new Date(`${selectedDate}T00:00:00`).toLocaleDateString("ja-JP", { month: "long", day: "numeric", weekday: "long" })}の予定</h2>{selectedItems.length ? <div className="mt-4 divide-y divide-white/10">{selectedItems.map((item) => <div key={item.id}><ScheduleCard item={item} />{item.details && <p className="mb-4 ml-[72px] whitespace-pre-wrap text-sm leading-6 text-white/55">{item.details}</p>}</div>)}</div> : <p className="mt-4 text-sm text-white/35">この日の予定はありません</p>}</section>
  </>;
}
