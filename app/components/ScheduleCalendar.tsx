...
function itemDot(item: ScheduleItem) { return colors[item.schedule_type] ?? colors.other; }
...
export default function ScheduleCalendar({ items, periods, applications, currentTime, canManage = false, coachId, initialSelectedDate }: { items: ScheduleItem[]; periods: SchedulePeriod[]; applications: CompetitionApplicationItem[]; currentTime: string; canManage?: boolean; coachId?: string; initialSelectedDate?: string }) {
  ...
  const filteredPeriods = useMemo(() => [...periods].sort((a, b) => b.starts_on.localeCompare(a.starts_on)), [periods]);
  const periodForDate = (key: string) => filteredPeriods.find((period) => period.starts_on <= key && period.ends_on >= key);
  ...
  function moveWeek(amount: number) { const next = new Date(selectedDay.getFullYear(), selectedDay.getMonth(), selectedDay.getDate() + amount * 7); setSelectedDate(dateKey(next)); setMonth(startOfMonth(next)); }
  ...
  return <>
    ...
    <section className="mt-8 rounded-2xl border border-white/10 bg-[#111] p-4 text-white sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-black"><CalendarDays className="text-orange-400" />月間カレンダー</h2><button type="button" onClick={goToday} className="rounded-lg border border-white/15 px-3 py-2 text-xs font-bold text-white/60">今日へ戻る</button></div>
      ...
      <div className="mt-4 sm:hidden"><div className="mb-2 flex items-center justify-between"><button type="button" onClick={() => moveWeek(-1)} aria-label="前の週" className="rounded-lg border border-white/10 p-2 text-white/55"><ChevronLeft size={18}/></button><span className="text-xs font-black text-white/55">{mobileWeekStart.getMonth() + 1}/{mobileWeekStart.getDate()} からの1週間</span><button type="button" onClick={() => moveWeek(1)} aria-label="次の週" className="rounded-lg border border-white/10 p-2 text-white/55"><ChevronRight size={18}/></button></div><div className="grid grid-cols-7 gap-1">{mobileWeekDays.map((day, index) => { const key = dateKey(day); const dayItems = byDate[key] ?? []; const period = periodForDate(key); const theme = schedulePhase(period?.phase); const selected = key === selectedDate; const isToday = key === dateKey(today); return <button type="button" key={key} onClick={() => { setSelectedDate(key); setMonth(startOfMonth(day)); }} className={`min-w-0 rounded-xl border px-1 py-2 text-center ${selected ? "border-orange-400 bg-orange-500/15 ring-1 ring-orange-400" : "border-white/10 bg-white/[.025]"}`}><span className={`block text-[9px] font-black ${index === 0 ? "text-red-300" : index === 6 ? "text-sky-300" : "text-white/40"}`}>{weekdays[index]}</span><span className={`mx-auto mt-1 grid h-7 w-7 place-items-center rounded-full text-sm font-black ${isToday ? "bg-orange-500 text-black" : ""}`}>{day.getDate()}</span><span className="mt-1 flex min-h-2 justify-center gap-0.5">{dayItems.slice(0, 2).map((item) => <i key={item.id} className={`h-1.5 w-1.5 rounded-full ${itemDot(item)}`}/>)}{period ? <i className={`h-1.5 w-1.5 rounded-full ${theme.dot}`}/> : null}</span></button>; })}</div><p className="mt-3 text-center text-xs font-bold text-orange-300">日付を選ぶと、すぐ下に予定が表示されます</p></div>
      <div className="mt-3 hidden text-center text-[10px] font-bold text-white/35 sm:grid" style={{ gridTemplateColumns: "repeat(7, minmax(0, 1fr))" }}>{weekdays.map((day, index) => <span key={day} className={index === 0 ? "text-red-300/60" : index === 6 ? "text-sky-300/60" : ""}>{day}</span>)}</div>
      <div className="mt-2 hidden gap-1 sm:grid" style={{ gridTemplateColumns: "repeat(7, minmax(0, 1fr))" }}>{days.map((day) => { const key = dateKey(day); const dayItems = byDate[key] ?? []; const period = periodForDate(key); const theme = schedulePhase(period?.phase); const inMonth = day.getMonth() === month.getMonth(); const selected = key === selectedDate; const isToday = key === dateKey(today); return <button type="button" key={key} onClick={() => setSelectedDate(key)} className={`h-20 min-w-0 overflow-hidden rounded-lg border p-2 text-left transition ${selected ? "ring-2 ring-orange-400" : ""} ${theme.day} ${inMonth ? "text-white" : "text-white/20"}`}><span className="flex items-center justify-between gap-1"><span className={`text-xs ${isToday ? "grid h-5 w-5 place-items-center rounded-full bg-orange-500 font-black text-black" : ""}`}>{day.getDate()}</span>{period && <i className={`h-1.5 w-1.5 rounded-full ${theme.dot}`} />}</span>{period && <span className="mt-0.5 block truncate text-[8px] font-bold opacity-70">{period.label || theme.label}</span>}<span className="mt-1 block space-y-0.5">{dayItems.slice(0, 2).map((item) => <span key={item.id} className="flex min-w-0 items-center gap-1"><i className={`block h-1.5 w-1.5 shrink-0 rounded-full ${itemDot(item)}`} /><span className="truncate text-[9px] text-white/60">{item.title}</span></span>)}{dayItems.length > 2 && <span className="block text-[9px] text-white/35">＋{dayItems.length - 2}件</span>}</span></button>; })}</div>
      <div className="mt-4 flex flex-wrap gap-3 text-[10px] text-white/45">{Object.entries(labels).filter(([key]) => key !== "all").map(([key, label]) => <span key={key} className="flex items-center gap-1"><i className={`h-2 w-2 rounded-full ${colors[key]}`} />{label}</span>)}</div>{periods.length ? <div className="mt-2 flex flex-wrap gap-3 border-t border-white/[0.06] pt-2 text-[10px] text-white/45">{schedulePhases.slice(1).map((phase) => <span key={phase.value} className="flex items-center gap-1"><i className={`h-2 w-2 rounded-full ${phase.dot}`} />{phase.label}</span>)}</div> : null}
    </section>
    <section className="mt-5 rounded-2xl border border-white/10 bg-[#111] p-5 text-white sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-black">{new Date(`${selectedDate}T00:00:00`).toLocaleDateString("ja-JP", { month: "long", day: "numeric", weekday: "long" })}の予定</h2><div className="flex flex-wrap gap-2">{canManage ? <Link href={`/mypage/schedules?newDate=${selectedDate}#schedule-management`} className="inline-flex items-center gap-1.5 rounded-lg bg-orange-500 px-3 py-2 text-xs font-black text-black"><Plus size={15}/>この日に追加</Link> : null}</div></div>{selectedItems.length ? <div className="mt-4 divide-y divide-white/10">{selectedItems.map((item) => <div key={item.id} className="pb-4"><ScheduleCard item={item} />{canManage && item.author_id === coachId ? <div className="mb-3 ml-[72px] flex flex-wrap gap-2"><Link href={`/mypage/schedules?edit=${item.id}#schedule-management`} className="inline-flex items-center gap-1.5 rounded-lg border border-orange-500/35 px-3 py-2 text-xs font-bold text-orange-300"><Pencil size={14}/>この予定を編集</Link><button type="button" disabled={deletingId === item.id} onClick={() => removeSchedule(item)} className="inline-flex items-center gap-1.5 rounded-lg border border-red-500/35 px-3 py-2 text-xs font-bold text-red-300 disabled:opacity-40"><Trash2 size={14}/>{deletingId === item.id ? "削除中…" : "この予定を削除"}</button></div> : null}{item.details && <p className="mb-4 ml-[72px] whitespace-pre-wrap text-sm leading-6 text-white/55">{item.details}</p>}{item.schedule_type === "competition" && item.registration_enabled ? <div className="ml-[72px]"><CompetitionApplication scheduleId={item.id} opensAt={item.registration_opens_at} deadline={item.registration_deadline} currentTime={currentTime} initialApplication={applications.find((application) => application.schedule_id === item.id)}/></div> : null}</div>)}</div> : <p className="mt-4 text-sm text-white/35">この日の予定はありません</p>}</section>
  </>;
}
