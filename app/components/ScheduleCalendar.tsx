...
import { CalendarDays, ChevronLeft, ChevronRight, Pencil, Plus, Trophy } from "lucide-react";
...
export default function ScheduleCalendar({ items, applications, currentTime, canManage = false, coachId, initialSelectedDate }: { items: ScheduleItem[]; applications: CompetitionApplicationItem[]; currentTime: string; canManage?: boolean; coachId?: string; initialSelectedDate?: string }) {
  ...
  return <>
    ...
    <section className="mt-8 rounded-2xl border border-white/10 bg-[#111] p-4 text-white sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-black"><CalendarDays className="text-orange-400" />月間カレンダー</h2><button type="button" onClick={goToday} className="rounded-lg border border-white/15 px-3 py-2 text-xs font-bold text-white/60">今日へ戻る</button></div>
      ...
      <div className="mt-2 grid gap-1" style={{ gridTemplateColumns: "repeat(7, minmax(0, 1fr))" }}>{days.map((day) => { const key = dateKey(day); const dayItems = byDate[key] ?? []; const inMonth = day.getMonth() === month.getMonth(); const selected = key === selectedDate; const isToday = key === dateKey(today); return <button type="button" key={key} onClick={() => setSelectedDate(key)} className={`h-14 min-w-0 overflow-hidden rounded-lg border p-1 text-left transition sm:h-20 sm:p-2 ${selected ? "border-orange-400 bg-orange-500/10" : "border-white/[0.06] bg-white/[0.015]"} ${inMonth ? "text-white" : "text-white/20"}`}><span className={`text-xs ${isToday ? "grid h-5 w-5 place-items-center rounded-full bg-orange-500 font-black text-black" : ""}`}>{day.getDate()}</span><span className="mt-1 flex flex-wrap gap-1 sm:hidden">{dayItems.slice(0, 3).map((item) => <i key={item.id} className={`block h-1.5 w-1.5 rounded-full ${colors[item.schedule_type] ?? colors.other}`} />)}</span><span className="mt-1 hidden space-y-0.5 sm:block">{dayItems.slice(0, 2).map((item) => <span key={item.id} className="flex min-w-0 items-center gap-1"><i className={`block h-1.5 w-1.5 shrink-0 rounded-full ${colors[item.schedule_type] ?? colors.other}`} /><span className="truncate text-[9px] text-white/60">{item.title}</span></span>)}{dayItems.length > 2 && <span className="block text-[9px] text-white/35">＋{dayItems.length - 2}件</span>}</span></button>; })}</div>
      <div className="mt-4 flex flex-wrap gap-3 text-[10px] text-white/45">{Object.entries(labels).filter(([key]) => key !== "all").map(([key, label]) => <span key={key} className="flex items-center gap-1"><i className={`h-2 w-2 rounded-full ${colors[key]}`} />{label}</span>)}</div>
    </section>
    <section className="mt-5 rounded-2xl border border-white/10 bg-[#111] p-5 text-white sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-black">{new Date(`${selectedDate}T00:00:00`).toLocaleDateString("ja-JP", { month: "long", day: "numeric", weekday: "long" })}の予定</h2>{canManage ? <Link href={`/mypage/schedules?newDate=${selectedDate}#schedule-management`} className="inline-flex items-center gap-1.5 rounded-lg bg-orange-500 px-3 py-2 text-xs font-black text-black"><Plus size={15}/>この日に追加</Link> : null}</div>{selectedItems.length ? <div className="mt-4 divide-y divide-white/10">{selectedItems.map((item) => <div key={item.id} className="pb-4"><ScheduleCard item={item} />{canManage && item.author_id === coachId ? <div className="mb-3 ml-[72px]"><Link href={`/mypage/schedules?edit=${item.id}#schedule-management`} className="inline-flex items-center gap-1.5 rounded-lg border border-orange-500/35 px-3 py-2 text-xs font-bold text-orange-300"><Pencil size={14}/>この予定を編集</Link></div> : null}{item.details && <p className="mb-4 ml-[72px] whitespace-pre-wrap text-sm leading-6 text-white/55">{item.details}</p>}{item.schedule_type === "competition" && item.registration_enabled ? <div className="ml-[72px]"><CompetitionApplication scheduleId={item.id} opensAt={item.registration_opens_at} deadline={item.registration_deadline} currentTime={currentTime} initialApplication={applications.find((application) => application.schedule_id === item.id)}/></div> : null}</div>)}</div> : <p className="mt-4 text-sm text-white/35">この日の予定はありません</p>}</section>
  </>;
}
