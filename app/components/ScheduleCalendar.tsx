...
export default function ScheduleCalendar({ items, periods, applications, currentTime, canManage = false, coachId, userId, initialSelectedDate }: { items: ScheduleItem[]; periods: SchedulePeriod[]; applications: CompetitionApplicationItem[]; currentTime: string; canManage?: boolean; coachId?: string; userId: string; initialSelectedDate?: string }) {
  ...
  const selectedPeriod = periodForDate(selectedDate);
  ...
  async function removePeriod(period: SchedulePeriod) {
    if (period.author_id !== userId || !confirm(`「${period.label || schedulePhase(period.phase).label}」の期間カラーを削除しますか？\n予定自体は削除されません。`)) return;
    const supabase = createClient();
    const { data, error } = await supabase.from("schedule_periods").delete().eq("id", period.id).eq("author_id", userId).select("id").maybeSingle();
    if (error || !data) { alert(error?.message ?? "期間カラーを削除できませんでした。"); return; }
    router.refresh();
  }
  return <>
    ...
    <section className="mt-8 rounded-2xl border border-white/10 bg-[#111] p-4 text-white sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="flex items-center gap-2 font-black"><CalendarDays className="text-orange-400" />月間カレンダー</h2><button type="button" onClick={goToday} className="rounded-lg border border-white/15 px-3 py-2 text-xs font-bold text-white/60">今日へ戻る</button></div>
      ...
      <div className="mt-2 grid gap-1" style={{ gridTemplateColumns: "repeat(7, minmax(0, 1fr))" }}>{days.map((day) => { const key = dateKey(day); const dayItems = byDate[key] ?? []; const period = periodForDate(key); const theme = schedulePhase(period?.phase); const inMonth = day.getMonth() === month.getMonth(); const selected = key === selectedDate; const isToday = key === dateKey(today); return <button type="button" key={key} onClick={() => setSelectedDate(key)} className={`h-14 min-w-0 overflow-hidden rounded-lg border p-1 text-left transition sm:h-20 sm:p-2 ${selected ? "ring-2 ring-orange-400" : ""} ${theme.day} ${inMonth ? "text-white" : "text-white/20"}`}><span className="flex items-center justify-between gap-1"><span className={`text-xs ${isToday ? "grid h-5 w-5 place-items-center rounded-full bg-orange-500 font-black text-black" : ""}`}>{day.getDate()}</span>{period && <i className={`h-1.5 w-1.5 rounded-full ${theme.dot}`} />}</span>{period && <span className="mt-0.5 hidden truncate text-[8px] font-bold opacity-70 sm:block">{period.label || theme.label}</span>}<span className="mt-1 flex flex-wrap gap-1 sm:hidden">{dayItems.slice(0, 3).map((item) => <i key={item.id} className={`block h-1.5 w-1.5 rounded-full ${itemDot(item)}`} />)}</span><span className="mt-1 hidden space-y-0.5 sm:block">{dayItems.slice(0, 2).map((item) => <span key={item.id} className="flex min-w-0 items-center gap-1"><i className={`block h-1.5 w-1.5 shrink-0 rounded-full ${itemDot(item)}`} /><span className="truncate text-[9px] text-white/60">{item.title}</span></span>)}{dayItems.length > 2 && <span className="block text-[9px] text-white/35">＋{dayItems.length - 2}件</span>}</span></button>; })}</div>
    ...
    <section className="mt-5 rounded-2xl border border-white/10 bg-[#111] p-5 text-white sm:p-6"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="font-black">{new Date(`${selectedDate}T00:00:00`).toLocaleDateString("ja-JP", { month: "long", day: "numeric", weekday: "long" })}の予定</h2><div className="flex flex-wrap gap-2"><Link href={`/mypage/schedules?periodDate=${selectedDate}#period-management`} className="inline-flex items-center gap-1.5 rounded-lg border border-orange-500/40 px-3 py-2 text-xs font-black text-orange-300"><Plus size={15}/>期間カラー</Link>{canManage ? <Link href={`/mypage/schedules?newDate=${selectedDate}#schedule-management`} className="inline-flex items-center gap-1.5 rounded-lg bg-orange-500 px-3 py-2 text-xs font-black text-black"><Plus size={15}/>この日に追加</Link> : null}</div></div>{selectedPeriod && (() => { const theme = schedulePhase(selectedPeriod.phase); return <div className={`mt-4 rounded-xl border px-4 py-3 ${theme.badge}`}><div className="flex flex-wrap items-center justify-between gap-3"><div><strong className="text-sm">{selectedPeriod.label || theme.label}</strong><span className="ml-2 text-[11px] opacity-70">{selectedPeriod.starts_on.replaceAll("-", "/")}〜{selectedPeriod.ends_on.replaceAll("-", "/")}</span></div><div className="flex gap-2"><Link href={`/mypage/schedules?period=${selectedPeriod.id}#period-management`} className="inline-flex items-center gap-1 rounded-lg border border-current/30 px-2.5 py-1.5 text-xs font-bold"><Pencil size={13}/>編集</Link><button type="button" onClick={() => removePeriod(selectedPeriod)} className="inline-flex items-center gap-1 rounded-lg border border-red-400/30 px-2.5 py-1.5 text-xs font-bold text-red-300"><Trash2 size={13}/>削除</button></div></div></div>; })()}{selectedItems.length ? <div className="mt-4 divide-y divide-white/10">{selectedItems.map((item) => <div key={item.id} className="pb-4"><ScheduleCard item={item} />{canManage && item.author_id === coachId ? <div className="mb-3 ml-[72px] flex flex-wrap gap-2"><Link href={`/mypage/schedules?edit=${item.id}#schedule-management`} className="inline-flex items-center gap-1.5 rounded-lg border border-orange-500/35 px-3 py-2 text-xs font-bold text-orange-300"><Pencil size={14}/>この予定を編集</Link><button type="button" disabled={deletingId === item.id} onClick={() => removeSchedule(item)} className="inline-flex items-center gap-1.5 rounded-lg border border-red-500/35 px-3 py-2 text-xs font-bold text-red-300 disabled:opacity-40"><Trash2 size={14}/>{deletingId === item.id ? "削除中…" : "この予定を削除"}</button></div> : null}{item.details && <p className="mb-4 ml-[72px] whitespace-pre-wrap text-sm leading-6 text-white/55">{item.details}</p>}{item.schedule_type === "competition" && item.registration_enabled ? <div className="ml-[72px]"><CompetitionApplication scheduleId={item.id} opensAt={item.registration_opens_at} deadline={item.registration_deadline} currentTime={currentTime} initialApplication={applications.find((application) => application.schedule_id === item.id)}/></div> : null}</div>)}</div> : <p className="mt-4 text-sm text-white/35">この日の予定はありません</p>}</section>
  </>;
}
