...
const goalOutcomeLabels: Record<string, string> = { achieved: "達成した", not_achieved: "惜しくも未達成", no_entry: "大会に出られなかった", changed: "目標を変更した" };
const goalNextActionLabels: Record<string, string> = { continue: "同じ目標を継続", new_goal: "新しい目標へ" };
...
export default function MyCalendar({ userId, initialEntries, schedules, activeScheduleIds, records, periods, initialGoal, goalHistory, initialSelectedDate, initialPeriodId, initialPeriodDate }: { userId: string; initialEntries: Entry[]; schedules: ClubSchedule[]; activeScheduleIds: number[]; records: PerformanceRecord[]; periods: SchedulePeriod[]; initialGoal: CalendarGoal | null; goalHistory: CalendarGoal[]; initialSelectedDate?: string; initialPeriodId?: number | null; initialPeriodDate?: string }) {
  ...
  const calendarGoals = useMemo(() => goal ? [goal, ...goalHistory.filter((item) => item.id !== goal.id)] : goalHistory, [goal, goalHistory]);
  const selectedGoals = calendarGoals.filter((item) => item.target_date === selectedDate);
  ...
  return <div className="mt-8 grid items-start gap-5 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,.75fr)]">
      ...
      {mobileCalendarView === "week" ? <div className="mt-4 lg:hidden">
        ...
        <div className="mt-3 space-y-2">
          {mobileWeekDays.map((day, index) => {
            ...
            const goalDay = calendarGoals.some((item) => item.target_date === key);
    ...
    <section className="rounded-[26px] border border-white/10 bg-[#111] p-5 sm:p-6 xl:sticky xl:top-20">
      ...
      {selectedGoals.length ? <div className="mt-5 space-y-3">{selectedGoals.map((calendarGoal) => <article key={calendarGoal.id} className="rounded-2xl border border-orange-500/30 bg-orange-500/[.07] p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[9px] font-black tracking-[.18em] text-orange-300">{calendarGoal.status === "active" ? "CURRENT TARGET" : "TARGET REVIEW"}</p><h3 className="mt-1 text-lg font-black">{calendarGoal.title}</h3><p className="mt-1 text-xs text-white/45">{calendarGoal.event_name ? `${calendarGoal.event_name} ・ ` : ""}{calendarGoal.target_value ? `目標 ${calendarGoal.target_value}${calendarGoal.target_unit ?? ""}` : "記録目標なし"}</p></div><Flag size={18} className="shrink-0 fill-orange-400 text-orange-400"/></div>{calendarGoal.status !== "active" ? <div className="mt-4 space-y-3 border-t border-orange-500/15 pt-4"><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-orange-500/30 px-2.5 py-1 text-[10px] font-black text-orange-200">{goalOutcomeLabels[calendarGoal.outcome ?? ""] ?? "振り返り済み"}</span>{calendarGoal.result_value != null ? <span className="text-sm font-black text-white">実際の記録 {calendarGoal.result_value}{calendarGoal.result_unit ?? ""}</span> : null}</div>{calendarGoal.reflection ? <div><p className="text-[10px] font-black tracking-[.12em] text-white/35">当時の振り返り</p><p className="mt-1 whitespace-pre-wrap text-sm leading-6 text-white/75">{calendarGoal.reflection}</p></div> : <p className="text-xs text-white/35">振り返りの文章はありません</p>}{calendarGoal.next_action ? <p className="text-xs font-bold text-orange-200/80">次の行動：{goalNextActionLabels[calendarGoal.next_action] ?? calendarGoal.next_action}</p> : null}</div> : <p className="mt-3 text-xs text-orange-200/70">この日に向けて取り組んでいる目標です。</p>}</article>)}</div> : null}
    ...
    <button type="button" onClick={startNew} aria-label="選択中の日に予定を追加" className="fixed bottom-5 right-5 z-40 grid h-14 w-14 place-items-center rounded-full bg-orange-500 text-black shadow-[0_12px_35px_rgba(249,115,22,.35)] sm:hidden"><Plus size={25}/></button>
    {open && <EntryEditor
      userId={userId}
      date={selectedDate}
      entry={editing}
    ...
    {reviewOpen && goal && <GoalReviewEditor
      ...
      onSaved={(continueGoal) => { setGoal(null); setReviewOpen(false); router.refresh(); if (continueGoal) setGoalOpen(true); }}
    />}
  </div>;
}
