...
const colors: Record<string, { dot: string; border: string; label: string }> = {
  ...
  sky: { dot: "bg-sky-400", border: "border-sky-500/40", label: "ブルー" },
  emerald: {
    dot: "bg-emerald-400",
  ...
  rose: { dot: "bg-rose-400", border: "border-rose-500/40", label: "レッド" },
  slate: {
    dot: "bg-slate-400",
...
const entryTypes = {
  school_practice: "学校練習",
  personal_training: "自主練習",
  competition: "試合",
  rest: "休養",
  other: "その他",
  club_schedule: "クラブ予定",
} as const;
const goalOutcomeLabels: Record<string, string> = {
  achieved: "達成した",
  not_achieved: "惜しくも未達成",
  no_entry: "大会に出られなかった",
  changed: "目標を変更した",
};
...
function entryTimeLabel(entry: Entry | undefined) {
  if (!entry || entry.all_day) return null;
  const start = timeValue(entry.starts_at);
  const end = timeValue(entry.ends_at);
  return start ? `${start}${end ? `〜${end}` : ""}` : null;
}
function scheduleDates(schedule: ClubSchedule) {
  const start = new Date(schedule.starts_at);
  const end = schedule.ends_at ? new Date(schedule.ends_at) : start;
...
export default function MyCalendar({
  ...
  const selectedDateValue = initialSelectedDate
    ? new Date(`${initialSelectedDate}T00:00:00`)
    : today;
  const [month, setMonth] = useState(
    new Date(selectedDateValue.getFullYear(), selectedDateValue.getMonth(), 1),
  );
  ...
  const displayItems = useMemo(() => {
    ...
    performanceRecords
      ...
      result.push({
        ...
        color:
          performance.record_kind === "athletics"
            ? "violet"
            : performance.record_kind === "control-test"
              ? "sky"
              : "emerald",
        performance,
        active: true,
  ...
  const periodForDate = (key: string) =>
    periods.find((period) => period.starts_on <= key && period.ends_on >= key);
  ...
  const calendarGoals = useMemo(
    () =>
      goal
        ? [goal, ...goalHistory.filter((item) => item.id !== goal.id)]
        : goalHistory,
    [goal, goalHistory],
  );
  const selectedGoals = calendarGoals.filter(
    (item) => item.target_date === selectedDate,
  );
  const selectedDay = new Date(`${selectedDate}T00:00:00`);
  const mobileWeekStart = new Date(
    selectedDay.getFullYear(),
    selectedDay.getMonth(),
    selectedDay.getDate() - selectedDay.getDay(),
  );
  const mobileWeekDays = Array.from(
    { length: 7 },
    (_, index) =>
      new Date(
        mobileWeekStart.getFullYear(),
        mobileWeekStart.getMonth(),
        mobileWeekStart.getDate() + index,
      ),
  );
  function startNew() {
    setEditing(null);
  ...
  async function saveWeekPlan() {
    ...
    try {
      ...
      const personalRows = rows.filter((row) => !row.scheduleId);
      const supabase = createClient();
      if (scheduleIds.length) {
        const { error: attendanceError } = await supabase.from("schedule_attendance").upsert(
  ...
  async function removePerformance(record: PerformanceRecord) {
    ...
    setPerformanceRecords((items) =>
      items.filter((item) => item.id !== record.id),
    );
    if (record.video_path) {
      const { error: storageError } = await supabase.storage
        .from(PERFORMANCE_VIDEO_BUCKET)
        .remove([record.video_path]);
      if (storageError)
        alert(
          "記録は削除しましたが、動画ファイルの削除に失敗しました。管理者へお知らせください。",
        );
    }
    router.refresh();
  }
  async function removePeriod(period: SchedulePeriod) {
    if (
      !confirm(
        `「${period.label || schedulePhase(period.phase).label}」の期間カラーを削除しますか？`,
      )
    )
      return;
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-4 py-3 text-sm font-black text-black"
            >
              <Flag size={16} />
              {goal ? "目標を変更" : "次の目標を設定"}
            </button>
            {goal ? (
              <button
                ...
                className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold text-white/60"
              >
                目標を振り返る
              </button>
            ) : null}
        ...
        <section className="rounded-[26px] border border-white/10 bg-[#111] p-4 sm:p-6">
          ...
          <div className="mt-3 grid grid-cols-2 rounded-xl border border-white/10 bg-black/25 p-1 lg:hidden">
            <button
              ...
              className={`rounded-lg px-3 py-2 text-xs font-black transition ${mobileCalendarView === "week" ? "bg-orange-500 text-black" : "text-white/45"}`}
            >
              週間
            </button>
            <button
              ...
              className={`rounded-lg px-3 py-2 text-xs font-black transition ${mobileCalendarView === "month" ? "bg-orange-500 text-black" : "text-white/45"}`}
            >
              1ヶ月
            </button>
          </div>
          {mobileCalendarView === "week" ? (
            <div className="mt-4 lg:hidden">
              <div className="flex items-center justify-between rounded-xl border border-white/10 bg-black/25 px-2 py-2">
                <button
                  ...
                  onClick={() => moveWeek(-1)}
                  aria-label="前の週"
                  className="rounded-lg p-2 text-white/55"
                >
                  <ChevronLeft size={19} />
                </button>
                <div className="text-center">
                  <p className="text-[9px] font-black tracking-[.16em] text-orange-400">
                    1週間の予定
                  </p>
                  <strong className="mt-0.5 block text-sm">
                    {mobileWeekStart.getMonth() + 1}/{mobileWeekStart.getDate()}
                    〜{mobileWeekDays[6].getMonth() + 1}/
                    {mobileWeekDays[6].getDate()}
                  </strong>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-3 space-y-2">
                {mobileWeekDays.map((day, index) => {
                  ...
                  const goalDay = calendarGoals.some(
                    (item) => item.target_date === key,
                  );
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`grid h-14 w-14 shrink-0 place-items-center rounded-xl border ${selected ? "border-orange-400/60 bg-orange-500/15" : "border-white/10 bg-black/25"}`}
                      >
                        <span
                          className={`block text-[9px] font-black ${index === 0 ? "text-red-300" : index === 6 ? "text-sky-300" : "text-white/45"}`}
                        >
                          {weekdays[index]}曜
                        </span>
                        <strong className="-mt-2 block text-xl leading-none">
                          {day.getDate()}
                        </strong>
                      </span>
                      <span className="min-w-0 flex-1">
                        <span className="flex flex-wrap items-center gap-1.5">
                          {goalDay ? (
                            <span className="inline-flex items-center gap-1 rounded-full bg-orange-500/15 px-2 py-1 text-[9px] font-black text-orange-300">
                              <Flag size={10} className="fill-orange-400" />
                              目標日
                            </span>
                          ) : null}
                          {period ? (
                            <span
                              className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[9px] font-black ${theme.badge}`}
                            >
                              <i
                        ...
                        {items.length > 0 ? (
                          <span className="mt-1.5 block space-y-1">
                            {items.slice(0, 2).map((item) => (
                              ...
                              >
                                <i
                                  className={`h-2 w-2 shrink-0 rounded-full ${colors[item.color]?.dot}`}
                                />
                            ...
                            {items.length > 2 ? (
                              <span className="block text-[10px] font-bold text-white/35">
                                ほか{items.length - 2}件
                              </span>
                            ) : null}
                          </span>
            ...
            <div className="mt-4 lg:hidden">
              <div className="grid grid-cols-7 text-center text-[9px] font-black text-white/35">
                  ...
                  return (
                    ...
                    >
                      <span className="flex items-center justify-between">
                        <strong className="text-[10px]">{day.getDate()}</strong>
                        {goalDay ? (
                          <Flag
                            size={8}
                      ...
                      {goalDay ? (
                        <span className="mt-1 block truncate text-[7px] font-black text-orange-300">
                          目標日
                        </span>
                      ) : period ? (
                        <span className="mt-1 block truncate text-[7px] font-bold opacity-70">
                          {period.label || theme.label}
                        </span>
                      ) : null}
                      {items.slice(0, 2).map((item) => (
                        ...
                        >
                          <i
                            className={`h-1 w-1 shrink-0 rounded-full ${colors[item.color]?.dot}`}
                          />
                          <span className="truncate text-[7px] text-white/55">
                            {item.title}
                          </span>
                        </span>
                      ))}
          ...
          <div className="mt-4 hidden grid-cols-7 text-center text-[10px] font-black text-white/35 lg:grid">
            {weekdays.map((day) => (
              <span key={day}>{day}</span>
            ))}
          </div>
          <div className="mt-2 hidden grid-cols-7 gap-1 lg:grid">
            {days.map((day) => {
              const key = dateKey(day);
              const items = byDate[key] ?? [];
              const period = periodForDate(key);
              const theme = schedulePhase(period?.phase);
              const current = day.getMonth() === month.getMonth();
              const calendarGoal = calendarGoals.find(
                (item) => item.target_date === key,
              );
                ...
                >
                  <span className="flex items-center justify-between">
                    ...
                    {goalDay ? (
                      ...
                      <i className={`h-1.5 w-1.5 rounded-full ${theme.dot}`} />
                    ) : null}
                  </span>
                  {goalDay ? (
                    <span className="mt-1 block truncate text-[8px] font-black text-orange-300">
                      目標：{calendarGoal?.title}
                    </span>
                  ) : period ? (
                    <span className="mt-0.5 block truncate text-[8px] font-bold opacity-70">
                      {period.label || theme.label}
                    </span>
                  ) : null}
                  <span className="mt-1 block space-y-1">
                    {items.slice(0, goalDay ? 2 : 3).map((item) => (
                      <span
                        key={item.key}
                        className="flex min-w-0 items-center gap-1"
                      >
                        <i
                          className={`h-1.5 w-1.5 shrink-0 rounded-full ${colors[item.color]?.dot}`}
                        />
          ...
          <div className="mt-4 flex flex-wrap gap-3 text-[10px] text-white/45">
            {schedulePhases.map((phase) => (
              <span key={phase.value} className="flex items-center gap-1">
                <i className={`h-2 w-2 rounded-full ${phase.dot}`} />
                {phase.label}
              </span>
            ))}
          </div>
        </section>
        <div id="period-management" className="scroll-mt-24">
          <SchedulePeriodManager
            key={
              initialPeriodId
                ? `edit-${initialPeriodId}`
                : initialPeriodDate
                  ? `new-${initialPeriodDate}`
                  : "closed"
            }
            initialPeriods={periods}
            userId={userId}
            initialEditingId={initialPeriodId}
              ...
              {weekPlanRows.map((row, index) => {
                ...
                return <div key={row.date} className="grid gap-2 rounded-2xl border border-white/[.08] bg-black/20 p-3 sm:grid-cols-[90px_150px_1fr] sm:items-center">
                  ...
                  <div className="grid gap-2">
                    {daySchedules.length ? <select aria-label={`${row.date}の全体スケジュール`} value={row.scheduleId ?? ""} onChange={(event) => setWeekPlanRows((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, scheduleId: event.target.value ? Number(event.target.value) : null } : item))} className="rounded-xl border border-orange-400/25 bg-orange-400/[.07] px-3 py-3 text-sm font-bold text-orange-100"><option value="">自分の予定を入力する</option>{daySchedules.map((schedule) => <option key={schedule.id} value={schedule.id}>全体予定：{schedule.title}{timeValue(schedule.starts_at) ? `（${timeValue(schedule.starts_at)}〜）` : ""}</option>)}</select> : null}
      ...
      {open && (
        <EntryEditor
          ...
          onSaved={(entry, performance) => {
            ...
            if (performance)
              setPerformanceRecords((items) => [
                ...items.filter((item) => item.id !== performance.id),
                performance,
              ]);
            close();
            router.refresh();
      ...
      {goalOpen && (
        <GoalEditor
          ...
          onSaved={(saved) => {
            setGoal(saved);
            setGoalOpen(false);
            setMonth(new Date(`${saved.target_date}T00:00:00`));
          }}
        />
      )}
      {reviewOpen && goal && (
        <GoalReviewEditor
          ...
          onSaved={(continueGoal) => {
            setGoal(null);
            setReviewOpen(false);
            router.refresh();
            if (continueGoal) setGoalOpen(true);
          }}
        />
      )}
      {quickPeriodOpen && (
  ...
  const label = scan ? "CONTROL TEST" : record
    ...
    : item.schedule
      ? item.active
        ? "参加予定"
        : "保存済みの振り返り"
      : (entryTypes[item.entry?.entry_type as keyof typeof entryTypes] ??
        "個人予定");
  const timeLabel = item.schedule
    ? item.schedule.all_day
      ? "終日"
      : `${timeValue(item.schedule.starts_at)}${item.schedule.ends_at ? `〜${timeValue(item.schedule.ends_at)}` : ""}`
    : entryTimeLabel(item.entry);

  return (
    <article
      className={`rounded-2xl border bg-black/20 p-4 ${colors[item.color]?.border}`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {timeLabel && (
            <p className="mt-1 text-xs font-bold text-orange-300/80">
              {timeLabel}
            </p>
          )}
          {(item.schedule?.location || item.entry?.location) && (
            <p className="mt-1 text-xs text-white/40">
              {item.schedule?.location || item.entry?.location}
            </p>
          )}
        ...
        {record ? (
          <div className="flex gap-1">
            ...
            <button
              onClick={() => void onRemovePerformance(record)}
        ...
        <div className="mt-3">
          ...
          <div className="mt-1.5 flex flex-wrap gap-1">
            {tags.map((tag) => (
              <span
                key={tag}
                className="rounded-full border border-orange-500/30 px-2 py-1 text-[10px] font-bold text-orange-300"
              >
                {tag}
              </span>
            ))}
      ...
      {(record?.awareness_note || item.entry?.journal) && (
        <div className="mt-3">
          <p className="text-[9px] font-black tracking-[.14em] text-white/35">
            練習の振り返り
          </p>
          <p className="mt-1.5 whitespace-pre-wrap text-sm leading-6 text-white/65">
            {record?.awareness_note || item.entry?.journal}
          </p>
        </div>
      )}
      {item.entry?.record_value && (
        <p className="mt-3 text-sm font-black text-emerald-300">
          記録 {item.entry.record_value}
          {item.entry.record_unit}
        </p>
      )}
      {item.entry?.performance_record_id && (
        <Link
          href={`/edit/${item.entry.performance_record_id}`}
          className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-sky-300"
        >
          <Link2 size={13} />
          パフォーマンス記録と連携
        </Link>
      ...
      {item.entry && (
        <button
          onClick={() => void onRemove(item.entry!)}
          className="mt-3 inline-flex items-center gap-1 text-[11px] font-bold text-red-300/70"
        >
          <Trash2 size={12} />
          個人記録を削除
        </button>
...
function GoalReviewEditor({
  ...
  goal: CalendarGoal;
  onClose: () => void;
  onSaved: (continueGoal: boolean) => void;
}) {
  ...
  async function save(event: FormEvent) {
    ...
    const { error: saveError } = await createClient()
      .from("personal_calendar_goals")
      .update({
        status: "completed",
        completed_at: new Date().toISOString(),
        outcome,
        result_value: resultValue ? Number(resultValue) : null,
        result_unit: resultValue ? resultUnit.trim() || null : null,
  ...
  return (
    <div className="fixed inset-0 z-[140] overflow-y-auto bg-black/80 p-4 backdrop-blur-sm">
      <form
        onSubmit={save}
        className="mx-auto my-12 max-w-lg rounded-[28px] border border-orange-500/40 bg-[#111] p-5 shadow-2xl sm:p-7"
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black">目標を振り返る</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="mt-5 space-y-4">
          <div>
            <span className="text-xs font-bold text-white/55">結果</span>
            <div className="mt-2 grid gap-2 sm:grid-cols-2">
              {Object.entries(goalOutcomeLabels).map(([value, label]) => (
                <button
                  type="button"
                  key={value}
                  onClick={() => setOutcome(value)}
                  className={`rounded-xl border px-3 py-3 text-left text-sm font-bold ${outcome === value ? "border-orange-400 bg-orange-500/10 text-orange-300" : "border-white/10 text-white/55"}`}
                >
                  {label}
                </button>
              ))}
            ...
            <div className="grid grid-cols-[1fr_120px] gap-3">
              <label>
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3"
                />
              </label>
              <label>
                <span className="text-xs font-bold text-white/55">単位</span>
                <input
                  maxLength={20}
                  value={resultUnit}
          ...
          <label className="block">
            ...
            <textarea
              ...
              placeholder="結果だけでなく、次につながる感覚を残す"
              className="mt-2 w-full resize-y rounded-xl border border-white/15 bg-black/30 px-4 py-3 leading-6"
            />
          </label>
          <div>
            <span className="text-xs font-bold text-white/55">
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              <button
                ...
                className={`rounded-xl border px-3 py-3 text-sm font-bold ${nextAction === "continue" ? "border-orange-400 bg-orange-500/10 text-orange-300" : "border-white/10 text-white/55"}`}
              >
                同じ目標を継続
              </button>
              <button
                type="button"
                onClick={() => setNextAction("new_goal")}
                className={`rounded-xl border px-3 py-3 text-sm font-bold ${nextAction === "new_goal" ? "border-orange-400 bg-orange-500/10 text-orange-300" : "border-white/10 text-white/55"}`}
              >
                新しい目標へ
              </button>
            </div>
        ...
        <div className="mt-6 flex justify-end gap-2">
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl border border-white/15 px-5 py-3 text-sm font-bold text-white/55"
          >
            閉じる
          </button>
          <button
            disabled={saving}
            className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-5 py-3 text-sm font-black text-black disabled:opacity-40"
          >
            <Check size={17} />
            {saving ? "保存中" : "振り返りを保存"}
          </button>
...
function GoalEditor({
  ...
  onClose: () => void;
  onSaved: (goal: CalendarGoal) => void;
}) {
  const calendarChoices = useMemo(() => {
    const choices = new Map<
      string,
      { value: string; title: string; dates: string[] }
    >();
    calendarItems.forEach((item) => {
      if (item.schedule && item.schedule.schedule_type !== "competition")
        return;
      if (!item.schedule && item.entry?.entry_type !== "competition") return;
      const value = item.schedule
    ...
    return [...choices.values()]
      .map((choice) => ({ ...choice, dates: choice.dates.sort() }))
      .sort((a, b) => a.dates[0].localeCompare(b.dates[0]));
  }, [calendarItems]);
  const initialSource = goal?.schedule_id
    ? `schedule:${goal.schedule_id}`
    : goal?.calendar_entry_id
      ? `entry:${goal.calendar_entry_id}`
      : "";
  const [calendarSource, setCalendarSource] = useState(initialSource);
  const [title, setTitle] = useState(goal?.title ?? "");
  const [targetDate, setTargetDate] = useState(
    goal?.target_date ?? initialDate,
  );
  ...
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const selectedChoice = calendarChoices.find(
    (choice) => choice.value === calendarSource,
  );
  ...
  async function save(event: FormEvent) {
    ...
    const row = {
      ...
      target_unit: targetValue ? targetUnit.trim() || null : null,
      schedule_id: sourceType === "schedule" ? Number(sourceId) : null,
      calendar_entry_id: sourceType === "entry" ? Number(sourceId) : null,
      updated_at: new Date().toISOString(),
    };
    const supabase = createClient();
    const query = goal
      ? supabase
          ...
          .eq("id", goal.id)
          .eq("user_id", userId)
      : supabase.from("personal_calendar_goals").insert(row);
      ...
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-black">次の目標を1つ決める</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <p className="mt-3 text-sm leading-6 text-white/45">
          マイカレンダーにある試合・大会から、次の目標を選んでください。
        </p>
        <div className="mt-6 space-y-4">
          <label className="block">
            <span className="text-xs font-bold text-white/55">試合・大会</span>
            <select
              required
              ...
              onChange={(event) => {
                ...
                const choice = calendarChoices.find(
                  (item) => item.value === value,
                );
                if (choice) {
                  setTitle(choice.title);
                  setTargetDate(choice.dates[0]);
                }
              }}
              className="mt-2 w-full rounded-xl border border-orange-500/45 bg-[#111] px-4 py-3"
            >
              <option value="">試合を選択</option>
              {calendarChoices.map((choice) => (
                <option key={choice.value} value={choice.value}>
                  {choice.dates[0].replaceAll("-", "/")}
                  {choice.dates.length > 1
                    ? `〜${choice.dates.at(-1)?.replaceAll("-", "/")}（${choice.dates.length}日間）`
                    : ""}
                  　{choice.title}
                </option>
              ))}
            ...
            {calendarChoices.length === 0 ? (
              <p className="mt-2 text-xs text-orange-300">
                先にマイカレンダーへ試合・大会を追加してください。
              </p>
            ) : null}
          </label>
          {selectedChoice && selectedChoice.dates.length > 1 ? (
            <div>
              <span className="text-xs font-bold text-white/55">
                何日目を目標にする？
              </span>
              <div className="mt-2 grid grid-cols-3 gap-2">
                {selectedChoice.dates.map((date, index) => (
                  <button
                    type="button"
                    key={date}
                    onClick={() => setTargetDate(date)}
                    className={`rounded-xl border px-3 py-3 text-sm font-black ${targetDate === date ? "border-orange-400 bg-orange-500/15 text-orange-300" : "border-white/10 text-white/50"}`}
                  >
                    <span className="block">{index + 1}日目</span>
                    <span className="mt-1 block text-[10px] font-bold opacity-65">
          ...
          <label className="block">
            <span className="text-xs font-bold text-white/55">目標名</span>
            <input
              required
              maxLength={120}
              value={title}
          ...
          <label className="block">
            <span className="text-xs font-bold text-white/55">目標日</span>
            <div className="mt-2 rounded-xl border border-white/15 bg-black/30 px-4 py-3 text-white/60">
              {targetDate
                ? targetDate.replaceAll("-", "/")
                : "試合を選択してください"}
              {selectedChoice &&
              selectedChoice.dates.length > 1 &&
              selectedDayIndex >= 0
                ? `（${selectedDayIndex + 1}日目）`
                : ""}
            </div>
          </label>
          <label className="block">
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <button
            disabled={saving || !calendarSource || !title.trim() || !targetDate}
            className="inline-flex items-center gap-2 rounded-xl bg-orange-500 px-5 py-3 text-sm font-black text-black disabled:opacity-40"
          >
            <Check size={17} />
            {saving ? "保存中" : "目標を保存"}
...
function EntryEditor({
  ...
  const initialDate = schedule
    ? dateKey(new Date(schedule.starts_at))
    : (entry?.entry_date ?? date);
  const linkedRecord = records.find(
    (record) => record.id === entry?.performance_record_id,
  );
  const unofficialEvents = eventNamesByKind("unofficial-athletics");
  const [recordKind, setRecordKind] = useState<
    "athletics" | "unofficial-athletics"
  >(
    linkedRecord?.record_kind === "athletics"
      ? "athletics"
      : "unofficial-athletics",
  );
  const [title, setTitle] = useState(schedule?.title ?? entry?.title ?? "");
  const [entryDate, setEntryDate] = useState(initialDate);
  const [type, setType] = useState(
    schedule ? "club_schedule" : (entry?.entry_type ?? "personal_training"),
  );
  const [location, setLocation] = useState(
    schedule?.location ?? entry?.location ?? "",
  );
  const [journal, setJournal] = useState(entry?.journal ?? "");
  const [tags, setTags] = useState(entry?.awareness_categories ?? []);
  const [color, setColor] = useState(
    entry?.color ?? (schedule ? "orange" : "sky"),
  );
  ...
  const [performanceId, setPerformanceId] = useState(
    entry?.performance_record_id?.toString() ?? "",
  );
  ...
  const [allDay, setAllDay] = useState(
    schedule?.all_day ?? entry?.all_day ?? true,
  );
  const [startTime, setStartTime] = useState(
    timeValue(schedule?.starts_at ?? entry?.starts_at),
  );
  const [endTime, setEndTime] = useState(
    timeValue(schedule?.ends_at ?? entry?.ends_at),
  );
  ...
  const pickerItems = useMemo(
    () =>
      calendarItems.reduce<Record<string, DisplayItem[]>>((all, item) => {
        (all[item.date] ??= []).push(item);
        return all;
      }, {}),
    [calendarItems],
  );
  const pickerFirst = new Date(
    pickerMonth.getFullYear(),
    pickerMonth.getMonth(),
    1 - pickerMonth.getDay(),
  );
  const pickerDays = Array.from(
    { length: 42 },
    (_, index) =>
      new Date(
        pickerFirst.getFullYear(),
        pickerFirst.getMonth(),
        pickerFirst.getDate() + index,
      ),
  );
  function selectEntryDate(key: string) {
    setEntryDate(key);
  ...
  async function save(event: FormEvent) {
    event.preventDefault();
    setError("");
    if (!schedule && !allDay && !startTime)
      return setError("開始時刻を入力してください。");
    if (!schedule && !allDay && endTime && endTime < startTime)
    ...
    try {
      ...
      const personalStart =
        !allDay && startTime ? `${entryDate}T${startTime}:00+09:00` : null;
      const personalEnd =
        !allDay && endTime ? `${entryDate}T${endTime}:00+09:00` : null;
      const row = {
        user_id: userId,
        schedule_id: schedule?.id ?? entry?.schedule_id ?? null,
        entry_date: entryDate,
        all_day: schedule?.all_day ?? allDay,
        starts_at: schedule?.starts_at ?? personalStart,
        ends_at: schedule?.ends_at ?? personalEnd,
        entry_type: type,
        title: title.trim(),
        ...
        performance_record_id: resolvedPerformanceId,
        video_path: videoPath,
        color,
        updated_at: new Date().toISOString(),
      };
      ...
      if (createdPerformanceId)
        await supabase
          .from("performance_records")
          .delete()
          .eq("id", createdPerformanceId)
          .eq("user_id", userId);
      if (video && videoPath && videoPath !== entry?.video_path)
        await supabase.storage
          .from(PERFORMANCE_VIDEO_BUCKET)
          .remove([videoPath]);
      setError(
  ...
  return (
    <div className="fixed inset-0 z-[120] overflow-y-auto bg-black/80 p-4 backdrop-blur-sm">
      <form
        onSubmit={save}
        className="mx-auto my-8 max-w-3xl rounded-[28px] border border-orange-500/35 bg-[#111] p-5 shadow-2xl sm:p-7"
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] font-black tracking-[.2em] text-orange-400">
              {schedule ? "CLUB SCHEDULE LOG" : "PERSONAL LOG"}
            </p>
        ...
        {!schedule && (
          <div className="mt-5 rounded-2xl border border-white/10 bg-black/20">
            <button
              ...
              className="flex w-full items-center justify-between gap-3 px-4 py-3 text-left text-sm font-black"
            >
              ...
              <ChevronRight
                size={17}
                className={`text-white/40 transition ${calendarOpen ? "rotate-90" : ""}`}
              />
            </button>
            {calendarOpen && (
              <div className="border-t border-white/10 p-3 sm:p-4">
                ...
                <div className="mt-2 grid grid-cols-7 text-center text-[9px] font-black text-white/35">
                  {weekdays.map((day) => (
                    <span key={day}>{day}</span>
                  ))}
                </div>
                <div className="mt-1 grid grid-cols-7 gap-1">
                  {pickerDays.map((day) => {
                    const key = dateKey(day);
                    const items = pickerItems[key] ?? [];
                    const current = day.getMonth() === pickerMonth.getMonth();
                    return (
                      <button
                        type="button"
                        key={key}
                        onClick={() => selectEntryDate(key)}
                        className={`min-h-12 overflow-hidden rounded-lg border p-1 text-left sm:min-h-16 ${key === entryDate ? "border-orange-400 bg-orange-500/10 ring-1 ring-orange-400" : "border-white/[.07] bg-white/[.02]"} ${current ? "text-white" : "text-white/20"}`}
                      >
                          ...
                          {items.slice(0, 2).map((item) => (
                            ...
                            >
                              <i
                                className={`h-1.5 w-1.5 shrink-0 rounded-full ${colors[item.color]?.dot}`}
                              />
                              <span className="hidden truncate text-[8px] text-white/50 sm:block">
                                {item.title}
                              </span>
                            </span>
                          ))}
                ...
                <p className="mt-3 text-center text-xs font-bold text-orange-300">
                  選択中：
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          <label className="sm:col-span-2">
            <span className="text-xs font-bold text-white/55">タイトル</span>
            <input
              required
              maxLength={120}
              ...
              className="mt-2 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 disabled:opacity-60"
            />
          </label>
          <label>
            <span className="text-xs font-bold text-white/55">日付</span>
            <input
              type="date"
              required
              disabled={Boolean(schedule)}
              value={entryDate}
              onChange={(e) => selectEntryDate(e.target.value)}
              className="mt-2 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 disabled:opacity-60"
            />
          </label>
          {!schedule && (
            <>
              <label className="flex items-center gap-2 rounded-xl border border-white/10 px-4 py-3 text-sm font-bold sm:col-span-2">
                <input
                  type="checkbox"
                  checked={allDay}
                ...
                <div className="grid gap-3 sm:col-span-2 sm:grid-cols-2">
                  <label>
                    ...
                    <input
                      ...
                      className="mt-2 w-full rounded-xl border border-orange-500/40 bg-black/30 px-4 py-3 [color-scheme:dark]"
                    />
                  </label>
                  <label>
                    <span className="text-xs font-bold text-white/55">
                    ...
                    <input
                      ...
                      className="mt-2 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 [color-scheme:dark]"
                    />
                  </label>
                </div>
              )}
          ...
          <label>
            <span className="text-xs font-bold text-white/55">種類</span>
            <select
              disabled={Boolean(schedule)}
              value={type}
              onChange={(e) => setType(e.target.value)}
              className="mt-2 w-full rounded-xl border border-white/15 bg-[#111] px-4 py-3 disabled:opacity-60"
            >
              {Object.entries(entryTypes)
                .filter(([key]) => key !== "club_schedule" || schedule)
                .map(([key, label]) => (
          ...
          <div className="sm:col-span-2">
            <span className="text-xs font-bold text-white/55">カラー</span>
            <div className="mt-2 flex flex-wrap gap-2">
              {Object.entries(colors).map(([key, item]) => (
                <button
                  ...
                  className={`h-9 w-9 rounded-full ${item.dot} ${color === key ? "ring-2 ring-white ring-offset-2 ring-offset-[#111]" : "opacity-55"}`}
                />
              ))}
            </div>
          </div>
          <div className="sm:col-span-2">
            ...
            <div className="mt-2">
              <AwarenessTagSelector value={tags} onChange={setTags} />
            </div>
          </div>
          <label className="sm:col-span-2">
            <span className="text-xs font-bold text-white/55">
            ...
            <textarea
              ...
              placeholder="やったこと、感覚、次に試したいこと"
              className="mt-2 w-full resize-y rounded-xl border border-white/15 bg-black/30 px-4 py-3 leading-6"
            />
          </label>
          <div className="sm:col-span-2">
            <span className="text-xs font-bold text-white/55">記録の種類</span>
            <div className="mt-2 grid grid-cols-2 gap-2 rounded-xl border border-white/10 bg-black/25 p-1">
              <button
                type="button"
                onClick={() => setRecordKind("unofficial-athletics")}
                className={`rounded-lg px-3 py-3 text-sm font-black transition ${recordKind === "unofficial-athletics" ? "bg-emerald-500 text-black" : "text-white/45"}`}
              >
                練習記録
              </button>
              <button
                ...
                className={`rounded-lg px-3 py-3 text-sm font-black transition ${recordKind === "athletics" ? "bg-orange-500 text-black" : "text-white/45"}`}
              >
                本番記録
              </button>
            </div>
          ...
          <label className="sm:col-span-2">
            ...
            >
              ...
              {unofficialEvents.map((eventName) => (
          ...
          <div>
            ...
            <input
              ...
              placeholder="例：7.25"
              className="mt-2 w-full rounded-xl border border-emerald-500/35 bg-black/30 px-4 py-3 disabled:opacity-35"
            />
          </div>
          <label>
            <span className="text-xs font-bold text-white/55">単位</span>
            <input
              ...
              placeholder="m / 秒 / kg"
              className="mt-2 w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 disabled:opacity-35"
            />
          </label>
          <p className="-mt-2 text-[11px] leading-5 text-emerald-300/70 sm:col-span-2">
            種目と記録を入力すると、意識・振り返り・動画と一緒に「
            {recordKind === "athletics" ? "本番記録" : "練習記録"}
            」へ自動で反映されます。
          </p>
          <label className="sm:col-span-2">
              ...
              onChange={(event) => {
                ...
                if (selected) {
                  ...
                  if (
                    ...
                    selected.record_kind === "unofficial-athletics"
                  )
                    setRecordKind(selected.record_kind);
                }
              }}
              className="mt-2 w-full rounded-xl border border-white/15 bg-[#111] px-4 py-3"
            >
              <option value="">
                新しい{recordKind === "athletics" ? "本番記録" : "練習記録"}
                として保存
              </option>
              {records
                .filter((record) => record.record_kind !== "control-test")
                .map((record) => (
                  <option key={record.id} value={record.id}>
                    {record.date}　
                    {record.record_kind === "athletics" ? "本番" : "練習"}　
          ...
          <div className="sm:col-span-2 rounded-xl border border-white/10 p-4">
            ...
            {entry?.video_path && !removeVideo && !video ? (
              <div className="mt-2 flex items-center justify-between rounded-lg bg-white/[.04] p-3 text-xs">
                <span>保存済みの動画</span>
                <button
                  type="button"
              ...
              <label className="mt-2 flex cursor-pointer items-center gap-3 rounded-lg border border-dashed border-white/15 p-4 text-sm text-white/55">
                <Upload size={18} />
                {video
                  ? `${video.name}（${formatVideoSize(video.size)}）`
                  : "写真フォルダから動画を選択"}
                <input
                  type="file"
                  accept="video/*"
