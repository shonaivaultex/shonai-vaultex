...
export default function PushNotificationButton() {
  ...
  async function sendTest() {
    ...
    try {
      ...
      if (!response.ok || result.sent < 1) throw new Error(result.error || `通知を送信できませんでした（登録${result.attempted ?? 0}台）。通知を一度OFFにして再登録してください。`);
