...
type Cell={a1?:string;a2?:string;status?:"measured"|"absent"|"foul"|"retest"};
...
export default function ControlTestSession({athletes,schedules}:{athletes:Athlete[];schedules:Schedule[]}) {
  ...
  const best=(item:Cell)=>{const values=[item.a1,item.a2].map(Number).filter((n)=>Number.isFinite(n)&&n>0);if(!values.length)return null;return definition.betterDirection==="lower"?Math.min(...values):Math.max(...values);};
  ...
  async function publish(){
    const payload=roster.map((athlete)=>({
      ...
      measurements:Object.entries(entries[athlete.id]??{}).flatMap(([code,item])=>{
        ...
        return value?[{testCode:code,value,attempt1:Number(item.a1)||null,attempt2:Number(item.a2)||null}]:[];
      }),
  ...
  return <div className="mt-8 space-y-5">
    ...
    <section className="overflow-hidden rounded-3xl border border-orange-500/35 bg-[#101010]"><div className="flex flex-wrap items-end justify-between gap-3 border-b border-white/10 p-5"><div><p className="text-[10px] font-black tracking-[.16em] text-orange-400">{definition.abilityEn}</p><h2 className="mt-1 text-2xl font-black">{definition.abilityJa}</h2><p className="mt-1 text-xs text-white/45">2本入力すると{definition.betterDirection==="lower"?"速い":"大きい"}方を自動採用</p></div><span className="flex items-center gap-2 text-xs text-emerald-300"><Cloud size={15}/>端末へ自動保存</span></div>
      <div className="divide-y divide-white/[.07]">{roster.length?roster.map((athlete)=>{const item=cell(athlete.id);const adopted=best(item);return <div key={athlete.id} className="grid gap-3 p-4 sm:grid-cols-[minmax(170px,1fr)_130px_130px_120px_180px] sm:items-center"><div><strong>{athlete.name}</strong><span className="ml-2 text-xs text-white/35">{athlete.programClass}</span><p className="mt-1 text-[11px] text-orange-300">{displayTest(athlete)}</p></div><input aria-label={`${athlete.name} 1本目`} inputMode="decimal" placeholder="1本目" value={item.a1??""} onChange={(e)=>setCell(athlete.id,{a1:e.target.value,status:"measured"})} className="rounded-xl border border-white/15 bg-black/30 px-3 py-3 text-lg font-black outline-none focus:border-orange-500"/><input aria-label={`${athlete.name} 2本目`} inputMode="decimal" placeholder="2本目" value={item.a2??""} onChange={(e)=>setCell(athlete.id,{a2:e.target.value,status:"measured"})} className="rounded-xl border border-white/15 bg-black/30 px-3 py-3 text-lg font-black outline-none focus:border-orange-500"/><div className="rounded-xl bg-white/[.05] px-3 py-3 text-center"><span className="text-[10px] text-white/35">採用</span><strong className="ml-2 text-orange-300">{adopted??"—"}</strong></div><select value={item.status??"measured"} onChange={(e)=>setCell(athlete.id,{status:e.target.value as Cell["status"],...(e.target.value!=="measured"?{a1:"",a2:""}:{})})} className="rounded-xl border border-white/10 bg-[#151515] px-3 py-3 text-xs text-white"><option value="measured">測定</option><option value="foul">ファウル</option><option value="retest">再測定</option><option value="absent">欠席・未測定</option></select></div>}):<p className="p-10 text-center text-sm text-white/35">参加者がいません。予定の出欠またはクラスを確認してください。</p>}</div>
    </section>
