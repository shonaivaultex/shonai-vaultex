...
export default async function PerformanceRecordsPage({ kind, title, eyebrow, description, selectedYear = null, focusRecordId = null, beforeRecords, addHref, addLabel = "記録を追加" }: Props) {
  ...
  const recordsQuery = supabase
    ...
    .select("id, category, value, wind_speed, date, record_kind, awareness_category, awareness_categories, awareness_note, video_path, advanced_details, performance_record_details(id, detail_type, sequence_number, round_name, value, wind_speed, place, status, video_path)")
    .eq("user_id", userId)
