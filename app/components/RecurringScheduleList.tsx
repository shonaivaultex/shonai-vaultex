...
type Props = { items: ScheduleItem[]; onEdit: (item: ScheduleItem) => void; onRemove: (item: ScheduleItem) => void; onRemoveMany: (items: ScheduleItem[]) => void };
type Group = { key: string; items: ScheduleItem[] };
const weekdays = ["日", "月", "火", "水", "木", "金", "土"];
function shortDate(value: string) { return new Date(value).toLocaleDateString("ja-JP", { month: "numeric", day: "numeric" }); }
function time(value: string) { return new Date(value).toLocaleTimeString("ja-JP", { hour: "2-digit", minute: "2-digit" }); }

export default function RecurringScheduleList({ items, onEdit, onRemove, onRemoveMany }: Props) {
  const [expanded, setExpanded] = useState<string[]>([]);
  const [openSection, setOpenSection] = useState<string | null>(null);
  const groups = useMemo(() => {
    const bySignature = items.reduce<Record<string, ScheduleItem[]>>((result, item) => { const key = [item.title, item.location ?? "", item.schedule_type, item.audience, item.program_class ?? "", time(item.starts_at)].join("|"); (result[key] ??= []).push(item); return result; }, {});
  ...
  return <div className="mt-5 space-y-3 border-t border-white/10 pt-5">{sections.map((section) => {
    ...
    return <section key={section.key} className="overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.01]">
      ...
      {isSectionOpen && <div className="divide-y divide-white/10 border-t border-white/[0.08] px-4">{section.groups.map((group) => { const first = group.items[0]; const last = group.items.at(-1)!; const recurring = group.items.length > 1; const isOpen = expanded.includes(group.key); return <div key={group.key} className="py-4"><div className="flex items-center gap-3"><button type="button" onClick={() => recurring && toggle(group.key)} className="min-w-0 flex-1 text-left"><strong className="block truncate text-sm">{first.title}</strong>{recurring ? <span className="mt-1 block text-xs text-white/45">毎週{weekdays[new Date(first.starts_at).getDay()]}曜 {time(first.starts_at)}・{shortDate(first.starts_at)}〜{shortDate(last.starts_at)}・全{group.items.length}回</span> : <span className="mt-1 block text-xs text-white/40">{new Date(first.starts_at).toLocaleString("ja-JP")}</span>}</button>{recurring ? <><button type="button" onClick={() => toggle(group.key)} aria-label="各日程を表示" className="p-2 text-white/55">{isOpen ? <ChevronUp size={18} /> : <ChevronDown size={18} />}</button><button type="button" onClick={() => onRemoveMany(group.items)} aria-label="繰り返し予定をまとめて削除" className="p-2 text-red-400"><Trash2 size={17} /></button></> : <><button type="button" onClick={() => onEdit(first)} aria-label={`${first.title}を編集`} className="p-2 text-white/60"><Pencil size={16} /></button><button type="button" onClick={() => onRemove(first)} aria-label={`${first.title}を削除`} className="p-2 text-red-400"><Trash2 size={16} /></button></>}</div>{recurring && isOpen && <div className="mt-3 ml-3 border-l border-white/10 pl-4">{group.items.map((item) => <div key={item.id} className="flex items-center gap-3 border-t border-white/[0.06] py-3 first:border-0"><span className="min-w-0 flex-1 text-xs text-white/50">{new Date(item.starts_at).toLocaleString("ja-JP")}</span><button type="button" onClick={() => onEdit(item)} aria-label="この日程を編集" className="p-2 text-white/50"><Pencil size={14} /></button><button type="button" onClick={() => onRemove(item)} aria-label="この日程を削除" className="p-2 text-red-400"><Trash2 size={14} /></button></div>)}</div>}</div>; })}</div>}
    </section>;
  })}</div>;
}
