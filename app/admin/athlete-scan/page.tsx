...
export default async function AthleteScanAdminPage() {
  ...
  const [{ data: standards }, { data: settings }, { data: scans }, { data: players }, { data: history }, { data: contactSettings }] = await Promise.all([
    ...
    admin.from("contact_profile_settings").select("*").eq("is_current", true).single(),
  ]);
  ...
  return <main className="min-h-screen bg-[#090a0c] px-4 pb-24 pt-28 text-white sm:px-8"><div className="mx-auto max-w-7xl">
    ...
    <AthleteStandardAdmin version={currentSet.version} label={currentSet.label} standards={(standards ?? []) as never} settings={(settings ?? { balanced_max_spread: 5, composite_max_gap: 8, type_descriptions: {} }) as never} scans={(scans ?? []) as never} players={(players ?? []) as never} history={(history ?? []) as never} contactSettings={contactSettings as never}/>
  </div></main>;
}
