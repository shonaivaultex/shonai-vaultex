...
export async function POST(request: Request) {
  ...
  const body = await request.json(); const kind = body.kind === "feedback" ? "feedback" : body.kind === "announcement" ? "announcement" : body.kind === "schedule" ? "schedule" : body.kind === "video_feedback" ? "video_feedback" : null; if (!kind) return NextResponse.json({ error: "Invalid request" }, { status: 400 });
  const pushQuery = kind === "video_feedback" ? supabase.rpc("get_video_feedback_push_targets", { p_request_id: body.requestId, p_sender_role: body.senderRole }) : supabase.rpc("get_push_targets", { p_kind: kind, p_record_id: body.recordId ?? null, p_audience: body.audience ?? null, p_program_class: body.programClass ?? null });
