...
export async function POST(request: Request) {
  ...
  const body = await request.json(); const kind = body.kind === "feedback" ? "feedback" : body.kind === "announcement" ? "announcement" : null; if (!kind) return NextResponse.json({ error: "Invalid request" }, { status: 400 });
  ...
  const payload = kind === "feedback" ? { title: "フィードバックが届きました", body: "コーチからのアドバイスを確認しましょう。", url: "/mypage", tag: `feedback-${body.recordId}` } : { title: body.title || "重要なお知らせ", body: body.body || "新しいお知らせがあります。", url: "/mypage", tag: "important-announcement" };
