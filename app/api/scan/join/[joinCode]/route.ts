...
export async function POST(request:NextRequest,{params}:{params:Promise<{joinCode:string}>}){
 ...
 if(body.action==="claim"){
  ...
  const token=randomAccessToken();const row={session_id:session.id,team_id:session.team_id,athlete_id:athleteId,pin_hash:pinHash,access_token_hash:hashParticipantSecret(token),last_saved_at:new Date().toISOString()};const {data:participant,error}=await admin.from("scan_session_participants").upsert(row,{onConflict:"session_id,athlete_id"}).select("status,draft_values").single();if(error)return NextResponse.json({error:error.message},{status:500});const results=participant.status==="submitted"?await getResults(admin,session,athlete):[];return NextResponse.json({ok:true,token,athlete,status:participant.status,values:participant.draft_values??{},results});
 }
 ...
 if(body.action==="results"){if(participant.status!=="submitted")return NextResponse.json({ok:true,submitted:false,results:[]});return NextResponse.json({ok:true,submitted:true,results:await getResults(admin,session,athlete)});}
 ...
 return NextResponse.json({ok:true,submitted:submitting,results:submitting?await getResults(admin,session,athlete):[]});
}
