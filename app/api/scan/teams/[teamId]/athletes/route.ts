...
export async function POST(request:NextRequest,{params}:{params:Promise<{teamId:string}>}){
 ...
 const body=await request.json() as {name?:unknown;grade?:unknown;event?:unknown};const name=typeof body.name==="string"?body.name.trim():"";if(!name)return NextResponse.json({error:"選手名を入力してください。"},{status:400});
 const {data,error}=await admin.from("scan_team_athletes").insert({team_id:teamId,name,grade:typeof body.grade==="string"?body.grade.trim()||null:null,event:typeof body.event==="string"?body.event.trim()||null:null}).select("*").single();
