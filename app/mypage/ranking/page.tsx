...
function GrowthPodiumCard({ row, unit }: { row: GrowthRankingRow; unit: string }) {
  ...
  return <article className={`relative rounded-3xl border p-5 text-center ${styles}`}>
    ...
    <span className="mt-2 block text-[10px] text-white/35">1年内の最初 {formatValue(row.starting_value, unit)} → 最新 {formatValue(row.latest_value, unit)}</span>
    {row.is_current_user ? <span className="mt-3 inline-block rounded-full bg-orange-400 px-2.5 py-1 text-[9px] font-black text-black">YOU</span> : null}
  </article>;
}
...
export default async function RankingPage({ searchParams }: { searchParams: Promise<{ category?: string; gender?: string; scope?: string; period?: string; mode?: string; kind?: string }> }) {
  ...
  const showsWindRule = isWindAffectedEvent(category);

  const today = new Date().toLocaleDateString("sv-SE", { timeZone: "Asia/Tokyo" });
  const { data, error } = mode === "growth"
    ? await supabase.rpc("get_yearly_growth_rankings", { p_category: category, p_record_kind: kind, p_as_of: today, p_gender: gender, p_scope: scope })
  ...
  return <main className="min-h-screen bg-[#090a0c] px-4 pb-28 pt-28 text-white sm:px-7">
    <div className="mx-auto max-w-6xl">
      ...
      <header className="mt-8 border-l-2 border-orange-500 pl-5">
        ...
        <p className="mt-3 max-w-2xl text-sm leading-7 text-white/50">{mode === "position" ? "本番記録だけで集計しています。順位は評価ではなく、次の目標を考えるための現在地です。" : "直近1年間の最初と最新の記録を比べ、長い目で見た成長を表示します。順位よりも、自分の変化を楽しむためのランキングです。"}</p>
      ...
      <nav className="mt-7 grid grid-cols-2 gap-2 rounded-2xl border border-white/10 bg-[#111] p-2" aria-label="ランキング種類">
        ...
        <Link href={rankingHref(category, gender, scope, period, "growth", kind)} className={`rounded-xl px-4 py-3 text-center text-sm font-black ${mode === "growth" ? "bg-emerald-400 text-black" : "text-white/45"}`}>年間の伸び率</Link>
      </nav>
      ...
      <nav className="mt-4 space-y-3" aria-label="ランキング条件">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          ...
          <Link href={rankingHref(category, gender, scope, "season", mode, kind)} className={`rounded-xl border px-3 py-3 text-center text-xs font-black ${period === "season" ? "border-orange-400/60 bg-orange-400/10 text-orange-300" : "border-white/10 text-white/45"}`}>{year}シーズン</Link></> : <div className="col-span-2 rounded-xl border border-white/10 px-3 py-3 text-center text-xs font-black text-white/35">直近1年の最初 → 最新</div>}
        </div>
      </nav>
    ...
    </aside> : mode === "growth" && currentGrowth ? <aside className="fixed inset-x-3 bottom-3 z-30 mx-auto max-w-2xl rounded-2xl border border-emerald-400/45 bg-[#0d1915]/95 p-4 shadow-2xl backdrop-blur sm:bottom-5"><div className="flex items-center gap-4"><TrendingUp className="shrink-0 text-emerald-300"/><div><span className="text-[9px] font-black tracking-[.16em] text-emerald-300">YOUR YEARLY GROWTH</span><strong className="mt-1 block">{currentGrowth.leaderboard_position}位 / {currentGrowth.total_count}人 ・ {Number(currentGrowth.growth_percent) > 0 ? "+" : ""}{Number(currentGrowth.growth_percent).toFixed(1)}%</strong></div></div></aside> : null}
  </main>;
}
