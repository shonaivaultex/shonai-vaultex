...
export default async function SchedulesPage({ searchParams }: { searchParams: Promise<{ newDate?: string; edit?: string; period?: string; periodDate?: string }> }) {
  ...
  const personalPeriods = periods.filter((item) => item.author_id === user.id);
  ...
  const selectedDate = initialDate ?? initialPeriodDate ?? (editingItem ? editingItem.starts_at.slice(0, 10) : undefined);
  const periodManagerKey = initialPeriodId ? `edit-${initialPeriodId}` : initialPeriodDate ? `new-${initialPeriodDate}` : "closed";
  return <main className="min-h-screen bg-[#090a0c] px-5 pb-20 pt-32 text-white sm:px-8"><div className={`mx-auto ${isCoach ? "max-w-7xl" : "max-w-3xl"}`}><Link href="/mypage" className="inline-flex items-center gap-2 text-xs font-bold tracking-[0.14em] text-white/60"><ArrowLeft size={16} />MY PAGE</Link><div className="mt-10 flex flex-wrap items-end justify-between gap-5"><header className="border-l-2 border-orange-500 pl-5"><p className="text-xs font-black tracking-[0.22em] text-orange-400">SCHEDULE</p><h1 className="mt-3 text-4xl font-black">スケジュール</h1><p className="mt-3 text-white/55">{isCoach ? "カレンダーを確認しながら、予定を直接追加・編集できます。" : "全体予定と所属クラスの予定を月ごとに確認できます。"}</p></header><Link href="/mypage/my-calendar" className="inline-flex items-center gap-2 rounded-xl border border-emerald-400/35 bg-emerald-400/10 px-4 py-3 text-sm font-black text-emerald-300 transition hover:bg-emerald-400/15"><CalendarRange size={18}/>マイカレンダーを開く</Link></div>{isCoach ? <div className="mt-8 grid items-start gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(420px,.85fr)]"><div><ScheduleCalendar key={selectedDate ?? "today"} items={items} periods={personalPeriods} applications={(applications ?? []) as CompetitionApplicationItem[]} currentTime={new Date().toISOString()} canManage coachId={user.id} userId={user.id} initialSelectedDate={selectedDate}/></div><div id="schedule-management" className="space-y-6 xl:sticky xl:top-20"><div id="period-management"><SchedulePeriodManager key={periodManagerKey} initialPeriods={personalPeriods} userId={user.id} initialEditingId={initialPeriodId} initialDate={initialPeriodDate}/></div><CoachScheduleManager key={`${initialEditingId ?? "new"}-${initialDate ?? "none"}`} initialItems={ownedItems} initialTemplates={templates} initialAttendance={attendance} competitionApplicants={competitionApplicants} initialDate={initialDate} initialEditingId={initialEditingId}/></div></div> : <><div id="period-management" className="mt-8"><SchedulePeriodManager key={periodManagerKey} initialPeriods={personalPeriods} userId={user.id} initialEditingId={initialPeriodId} initialDate={initialPeriodDate}/></div><ScheduleCalendar items={items} periods={personalPeriods} applications={(applications ?? []) as CompetitionApplicationItem[]} currentTime={new Date().toISOString()} userId={user.id}/></>}</div></main>;
}
