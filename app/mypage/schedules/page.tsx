...
export default async function SchedulesPage({ searchParams }: { searchParams: Promise<{ newDate?: string; edit?: string; attendance?: string }> }) {
  ...
  const [{ data }, { data: applications }, { data: coachRole }, { data: ownAttendanceRows }, { data: playerProfile }] = await Promise.all([
  ...
  const unansweredOnly = query.attendance === "unanswered";
  const attendanceRangeEnd = new Date(now);
  attendanceRangeEnd.setDate(attendanceRangeEnd.getDate() + 14);
  const answeredScheduleIds = new Set((ownAttendanceRows ?? []).map((row) => row.schedule_id));
  const memberCalendarItems = unansweredOnly
    ? items.filter((item) => !item.is_personal_slot && (item.audience === "all" || item.program_class === playerProfile?.program_class) && !answeredScheduleIds.has(item.id) && new Date(item.ends_at ?? item.starts_at) >= now && new Date(item.starts_at) <= attendanceRangeEnd)
    : items;
  ...
  return <main className="min-h-screen bg-[#090a0c] px-5 pb-20 pt-32 text-white sm:px-8"><div className={`mx-auto ${isCoach && !unansweredOnly ? "max-w-7xl" : "max-w-3xl"}`}><Link href="/mypage" className="inline-flex items-center gap-2 text-xs font-bold tracking-[0.14em] text-white/60"><ArrowLeft size={16} />MY PAGE</Link><div className="mt-10 flex flex-wrap items-end justify-between gap-5"><header className="border-l-2 border-orange-500 pl-5"><p className="text-xs font-black tracking-[0.22em] text-orange-400">SCHEDULE</p><h1 className="mt-3 text-4xl font-black">{unansweredOnly ? "未申込み・未回答の予定" : "クラブ予定・申込み"}</h1><p className="mt-3 text-white/55">{unansweredOnly ? "向こう2週間の、まだ申込み・回答をしていない予定です。練習会は参加希望の場合だけお申し込みください。" : isCoach ? "カレンダーを確認しながら、予定を直接追加・編集できます。" : "参加したい日の予定を開いて、時間・場所を確認して申し込みましょう。"}</p></header><Link href="/mypage/my-calendar" className="inline-flex items-center gap-2 rounded-xl border border-emerald-400/35 bg-emerald-400/10 px-4 py-3 text-sm font-black text-emerald-300 transition hover:bg-emerald-400/15"><CalendarRange size={18}/>マイカレンダーを開く</Link></div>{unansweredOnly ? <div className="mt-8 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-orange-400/25 bg-orange-400/[.07] p-5"><div><strong className="text-orange-300">2週間以内の未申込み・未回答 {memberCalendarItems.length}件</strong><p className="mt-1 text-sm text-white/50">申込み・回答をすると一覧から外れます。参加しない練習会への回答は不要です。</p></div><Link href="/mypage/schedules" className="rounded-full border border-white/15 px-4 py-2 text-xs font-black text-white/65 transition hover:border-white/30 hover:text-white">すべての予定を見る</Link></div> : null}{isCoach && !unansweredOnly ? <div className="mt-8 grid items-start gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(420px,.85fr)]"><div><ScheduleCalendar key={selectedDate ?? "today"} items={calendarItems} periods={[]} applications={(applications ?? []) as CompetitionApplicationItem[]} currentTime={new Date().toISOString()} canManage coachId={userId} initialSelectedDate={selectedDate} attendanceRosters={attendanceRosters}/></div><div id="schedule-management" className="space-y-6 xl:sticky xl:top-20"><CoachScheduleManager key={`${initialEditingId ?? "new"}-${initialDate ?? "none"}`} initialItems={ownedItems} initialTemplates={templates} initialAttendance={attendance} competitionApplicants={competitionApplicants} initialDate={initialDate} initialEditingId={initialEditingId}/></div></div> : <ScheduleCalendar key={unansweredOnly ? selectedDate ?? "unanswered" : "all"} items={memberCalendarItems} periods={[]} applications={(applications ?? []) as CompetitionApplicationItem[]} currentTime={new Date().toISOString()} initialSelectedDate={selectedDate}/>}</div></main>;
}
