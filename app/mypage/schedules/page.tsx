...
export default async function SchedulesPage() {
  ...
  const { data } = await supabase.from("schedules").select("*").gte("starts_at", rangeStart).lt("starts_at", rangeEnd).order("starts_at").limit(500);
  const items = (data ?? []) as ScheduleItem[];
  return <main className="min-h-screen bg-[#090a0c] px-5 pb-20 pt-32 text-white sm:px-8"><div className="mx-auto max-w-3xl"><Link href="/mypage" className="inline-flex items-center gap-2 text-xs font-bold tracking-[0.14em] text-white/60"><ArrowLeft size={16} />MY PAGE</Link><header className="mt-10 border-l-2 border-orange-500 pl-5"><p className="text-xs font-black tracking-[0.22em] text-orange-400">SCHEDULE</p><h1 className="mt-3 text-4xl font-black">スケジュール</h1><p className="mt-3 text-white/55">全体予定と所属クラスの予定を月ごとに確認できます。</p></header><ScheduleCalendar items={items} /></div></main>;
}
