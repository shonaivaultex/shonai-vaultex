...
export default async function VideoFeedbackPage() {
  ...
  const { data: messageReads } = coachMessageIds.length ? await supabase.from("video_feedback_message_reads").select("message_id").eq("user_id", user.id).in("message_id", coachMessageIds) : { data: [] };
  ...
  const items = await Promise.all((requests ?? []).map(async (item) => {
    ...
    return { ...item, video_url: data?.signedUrl ?? null, messages: (messages ?? []).filter((message) => message.request_id === item.id).map((message) => ({ ...message, read_by_athlete: message.sender_role === "coach" && readMessageIds.has(message.id) })) };
  }));
