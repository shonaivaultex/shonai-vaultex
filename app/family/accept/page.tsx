...
import { getFamilyInvitationAvailability, normalizeInvitationEmail } from "@/lib/family-invitation";
...
export default async function FamilyAcceptPage({ searchParams }: { searchParams: Promise<{ token?: string }> }) {
  ...
  const currentEmail = normalizeInvitationEmail(userData.user?.email);
  ...
  const availability = invitation ? getFamilyInvitationAvailability({ acceptedAt: invitation.accepted_at, expiresAt: invitation.expires_at, invitationEmail, currentEmail }) : "unavailable";
  const available = availability !== "unavailable";
  const emailMatches = availability === "available";
