...
export default async function CoachManagementPage() {
  ...
  const [{ data: attendance }, { data: competitionApplications }, { data: personalBookings }] = scheduleIds.length ? await Promise.all([
    ...
    supabase.from("personal_session_bookings").select("schedule_id,user_id,note,requested_location,created_at").in("schedule_id", scheduleIds).order("created_at"),
  ]) : [{ data: [] }, { data: [] }, { data: [] }];
