...
export default async function PerformanceSessionPage() {
  ...
  const [{ data: scheduleRows }, { data: athleteRows }] = await Promise.all([
    admin.from("schedules").select("id,title,starts_at,ends_at,schedule_type,program_class,schedule_attendance(user_id,status)").gte("starts_at", from.toISOString()).lte("starts_at", to.toISOString()).order("starts_at"),
  ...
  return <main className="min-h-screen bg-[#090a0c] px-4 pb-24 pt-28 text-white sm:px-8"><div className="mx-auto max-w-7xl">
    ...
    <header className="mt-7 border-l-2 border-orange-500 pl-5"><p className="text-xs font-black tracking-[.2em] text-orange-400">LIVE PERFORMANCE</p><h1 className="mt-2 text-3xl font-black sm:text-5xl">練習・大会記録 一括入力</h1><p className="mt-3 text-sm text-white/50">コーチが記録と動画を登録し、選手があとから意識や振り返りを追記できます。</p></header>
    <PerformanceSession athletes={athletes.map((a) => ({ id:a.user_id,name:a.name,programClass:a.program_class,event:a.event }))} schedules={schedules.map((s) => ({ id:s.id,title:s.title,startsAt:s.starts_at,endsAt:s.ends_at,type:s.schedule_type,programClass:s.program_class,attendeeIds:(s.schedule_attendance ?? []).filter((a) => a.status === "attending").map((a) => a.user_id) }))}/>
  </div></main>;
}
