...
export default async function CoachAthletePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ request?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#090a0c] px-5 pb-20 pt-32 text-white sm:px-8"><div className="mx-auto max-w-3xl">
    ...
    <div className="mt-8 space-y-5">{enriched.map((record) => { const request = requestByRecord.get(record.id); return <article id={`request-record-${record.id}`} key={record.id} className={`rounded-2xl bg-[#111] p-5 sm:p-6 ${focusRecordId === record.id ? "border border-sky-400 shadow-lg shadow-sky-500/10" : "border border-white/10"}`}>{request && <div className="mb-4 rounded-xl border border-sky-500/30 bg-sky-500/[0.08] p-4"><div className="flex items-center justify-between gap-2"><span className="text-xs font-black text-sky-300">フィードバック依頼：{requestLabels[request.request_type]}</span>{request.priority === "urgent" && <span className="rounded-full bg-red-500/15 px-2 py-1 text-[10px] font-bold text-red-300">大会前・優先</span>}</div>{request.message && <p className="mt-2 whitespace-pre-wrap text-sm text-white/75">{request.message}</p>}</div>}<div className="flex items-start justify-between gap-4"><div><span className="text-xs font-bold text-orange-400">{record.category}</span><strong className="mt-2 block text-3xl">{record.value}<span className="ml-1 text-sm text-white/50">{unitMap[record.category] ?? ""}</span></strong><span className="mt-1 block text-xs text-white/40">{record.date}</span></div>{record.awareness_category && <span className="rounded-full border border-orange-500/30 bg-orange-500/10 px-3 py-1 text-xs font-bold text-orange-300">{record.awareness_category}</span>}</div>{record.awareness_note && <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-white/65">{record.awareness_note}</p>}{record.video_url && <details className="mt-4" open={focusRecordId === record.id && request?.request_type === "video"}><summary className="cursor-pointer text-sm font-bold text-orange-400"><Play className="mr-2 inline" size={15} />動画を見る</summary><video controls playsInline className="mt-3 max-h-[65vh] w-full rounded-xl bg-black object-contain" src={record.video_url} /></details>}{(feedbackByRecord[record.id] ?? []).map((item) => <div key={item.id} className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-500/[0.06] p-4"><span className="text-xs font-bold text-emerald-300">コーチフィードバック</span><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-white/75">{item.body}</p><span className="mt-2 block text-xs text-white/30">{new Date(item.created_at).toLocaleString("ja-JP")}</span>{item.coach_id === user.id && <CoachFeedbackActions feedbackId={item.id} initialBody={item.body} />}</div>)}<CoachFeedbackForm recordId={record.id} requestId={request?.id ?? null} /></article>; })}</div>
  </div></main>;
}
