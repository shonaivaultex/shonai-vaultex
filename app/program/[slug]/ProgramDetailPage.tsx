...
<section className="border-y border-white/10 py-24 sm:py-32"><div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10"><motion.div {...reveal} className="grid gap-8 lg:grid-cols-12"><div className="lg:col-span-4"><SectionLabel>TRAINING FLOW</SectionLabel><h2 className="mt-6 text-4xl font-black tracking-[-0.045em] sm:text-5xl">YOUR SESSION.</h2></div><p className="max-w-md self-end text-sm leading-7 text-white/55 lg:col-span-5">初めにカウンセリングを行い、一人ひとりの思いや傾向を理解して練習方針を考えます。以下は練習の流れの一例です。人数・時期・会場・体調に合わせて、レクリエーションや同じ種目に取り組む時間も取り入れます。</p></motion.div><div className="mt-14 divide-y divide-white/10 border-y border-white/10">{program.flow.map((item) => <div key={item.time} className="grid gap-3 py-7 sm:grid-cols-[5rem_1fr_1.5fr] sm:gap-6"><p className="text-sm font-black text-orange-500">{item.time}</p><h3 className="text-xl font-black tracking-[-0.03em]">{item.title}</h3><p className="text-sm leading-7 text-white/60">{item.description}</p></div>)}</div></div></section>

<section className="py-24 sm:py-32"><motion.div {...reveal} className="mx-auto grid max-w-7xl gap-14 px-5 sm:px-8 lg:grid-cols-12 lg:gap-20 lg:px-10"><div className="lg:col-span-4"><SectionLabel>FAQ</SectionLabel><h2 className="mt-6 text-4xl font-black tracking-[-0.045em] sm:text-5xl">QUESTIONS.</h2></div><div className="divide-y divide-white/10 border-y border-white/10 lg:col-span-8">{program.faq.map((item, index) => <details key={item.question} className="group py-6"><summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-lg font-bold"><span><span className="mr-3 text-sm text-orange-500">0{index + 1}</span>{item.question}</span><span className="text-2xl font-normal text-orange-500 transition-transform group-open:rotate-45">+</span></summary><p className="max-w-2xl pt-5 text-sm leading-7 text-white/60">{item.answer}</p></details>)}</div></motion.div></section>

<section className="relative overflow-hidden bg-orange-500 py-24 text-[#090a0c] sm:py-32"><div className="pointer-events-none absolute -right-4 -top-16 select-none text-[10rem] font-black leading-none tracking-[-0.1em] text-black/10 sm:text-[16rem]">JOIN</div><motion.div {...reveal} className="relative mx-auto flex max-w-7xl flex-col justify-between gap-10 px-5 sm:px-8 md:flex-row md:items-end lg:px-10"><div><p className="text-xs font-black tracking-[0.28em]">JOIN {program.name}</p><h2 className="mt-5 text-4xl font-black leading-[1.05] tracking-[-0.05em] sm:text-5xl md:text-6xl">LET’S TALK.</h2></div><CtaLink href="https://line.me/R/ti/p/@082fhyco" variant="outline">公式LINEで相談する</CtaLink></motion.div></section>
  </main>;
}
